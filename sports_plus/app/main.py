...
NAV_ITEMS = [
    ...
    ("baseball.html", "⚾ 野球"),
    ("soccer.html", "⚽ サッカー"),
    ("basketball.html", "🏀 バスケット"),
    ("tennis.html", "🎾 テニス"),
    ("motorsports.html", "🏁 モータースポーツ"),
    ("combat.html", "🥊 格闘技"),
    ("analysis.html", "📊 分析"),
]

MAIN_SECTIONS = ["野球", "サッカー", "バスケット", "テニス", "モータースポーツ", "格闘技"]
CATEGORY_TO_FILE = {
    "野球": "baseball.html",
    "サッカー": "soccer.html",
    "バスケット": "basketball.html",
    "テニス": "tennis.html",
    "モータースポーツ": "motorsports.html",
    "格闘技": "combat.html",
}
CATEGORY_TO_ICON = {
    "野球": "⚾",
...
SPORT_LABELS = {"all": "全部", "baseball": "野球", "soccer": "サッカー", "basketball": "バスケット", "tennis": "テニス", "motorsports": "モータースポーツ", "combat": "格闘技"}
CATEGORY_KEYWORDS = {
    "野球": ["baseball", "mlb", "npb", "野球", "大谷", "ドジャース", "ヤンキース", "阪神", "巨人", "home run", "pitcher"],
    "サッカー": ["soccer", "football", "fifa", "uefa", "premier league", "laliga", "serie a", "bundesliga", "jリーグ", "サッカー", "ワールドカップ"],
    "バスケット": ["basketball", "nba", "wnba", "euroleague", "バスケット", "バスケ", "レイカーズ", "ウォリアーズ"],
    "テニス": ["tennis", "atp", "wta", "grand slam", "wimbledon", "roland garros", "us open", "全仏", "全豪", "テニス"],
    "モータースポーツ": ["formula 1", "f1", "motogp", "indycar", "nascar", "super gt", "motorsport", "モータースポーツ", "グランプリ"],
    "格闘技": ["mma", "ufc", "boxing", "boxer", "kickboxing", "wrestling", "柔道", "ボクシング", "総合格闘技", "格闘技"],
}
SPORT_TOPIC_KEYWORDS = {
    "試合・結果": ["match", "game", "result", "score", "win", "loss", "draw", "playoff", "final", "試合", "結果", "得点", "勝利", "敗戦", "引き分け", "プレーオフ", "決勝"],
    "移籍・契約": ["transfer", "contract", "signing", "deal", "trade", "loan", "extension", "移籍", "契約", "加入", "退団", "トレード", "レンタル", "延長"],
    "選手・チーム": ["player", "team", "coach", "manager", "captain", "roster", "lineup", "選手", "チーム", "監督", "コーチ", "主将", "ロスター", "スタメン"],
    "故障・欠場": ["injury", "injured", "absence", "out", "return", "rehab", "sidelined", "故障", "負傷", "欠場", "離脱", "復帰", "リハビリ"],
    "大会・リーグ": ["league", "tournament", "cup", "championship", "standings", "ranking", "season", "リーグ", "大会", "カップ", "選手権", "順位", "ランキング", "シーズン"],
    "ビジネス・放映権": ["broadcast", "media rights", "sponsor", "revenue", "ticket", "attendance", "放映権", "スポンサー", "収益", "チケット", "観客"],
}
POSITIVE_WORDS = ["record", "title", "champion", "winning", "tops", "首位", "優勝", "勝利", "新記録", "連勝", "best", "金メダル"]
CAUTION_WORDS = ["delay", "中止", "延期", "cancel", "controversy", "lawsuit", "problem", "不具合", "障害", "炎上"]
TREND_KEYWORDS = ["breaking", "速報", "final", "決勝", "transfer", "移籍", "trade", "record", "新記録", "injury", "負傷", "playoff", "優勝", "title", "announcement", "発表"]
...
def detect_category(title: str, summary: str, source_name: str, source_category: str = "") -> str:
    ...
    if best:
        return best
    return source_category if source_category in CATEGORY_KEYWORDS else "その他"
...
def detect_topic_tags(title: str, summary: str) -> list[str]:
    combined = text_lower(f"{title} {summary}")
    tags = [name for name, words in SPORT_TOPIC_KEYWORDS.items() if any(w in combined for w in words)]
    return tags or ["大会・リーグ"]
...
def calc_score(item: dict) -> int:
    ...
    for points, keywords in {
        3: ["record", "新記録", "優勝", "title", "champion", "決勝", "final", "playoff"],
        2: ["announcement", "発表", "transfer", "移籍", "trade", "契約", "injury", "負傷"],
        1: ["interview", "feature", "特集", "preview", "レビュー"],
    }.items():
...
def translate_items(items: list[dict], cache: dict) -> None:
    ...
    targets = non_ja_items if target_limit <= 0 else non_ja_items[:target_limit]
    if not targets:
        log("翻訳対象なし (日本語記事のみ)")
        return
    log(f"翻訳中... ({len(targets)}件)")
    sep = "\n\n---\n\n"
    ...
    def do_translate(item: dict) -> None:
        ...
        item["title_ja"] = t if t else item.get("title_ja", title)
        item["summary_ja"] = s if s else item.get("summary_ja", summary)
...
function markRead(btn, id) {
  safeStorageSet('watch_hub_plus_read_' + id, '1');
  var card = btn.closest('.item-card');
...
function syncFilterButtons() {
  document.querySelectorAll('.flt-region').forEach(function(btn){
    btn.classList.toggle('active', (btn.dataset.region || 'all') === window.__sportsRegion);
  });
  document.querySelectorAll('.flt-score').forEach(function(btn){
    btn.classList.toggle('active', (btn.dataset.score || 'all') === window.__sportsScore);
  });
}
function applySportsFilters() {
  var region = window.__sportsRegion || 'all';
  var score = window.__sportsScore || 'all';
  var search = (window.__sportsSearch || '').toLowerCase();
...
function setRegionFilter(region, btn) {
  window.__sportsRegion = region || 'all';
  applySportsFilters();
  if(btn && btn.blur) btn.blur();
}
function setScoreFilter(score, btn) {
  window.__sportsScore = score || 'all';
  applySportsFilters();
  if(btn && btn.blur) btn.blur();
}
function setSearchFilter(value) {
  window.__sportsSearch = value || '';
  applySportsFilters();
}
(function(){
  document.querySelectorAll('.item-card').forEach(function(card){
    var id = card.dataset.cardid;
    if(safeStorageGet('watch_hub_plus_read_' + id) === '1') card.classList.add('is-read');
  });
  setPageLang('en');
  var activeRegion = document.querySelector('.flt-region.active');
  var activeScore = document.querySelector('.flt-score.active');
  var sb = document.getElementById('searchBox');
  window.__sportsRegion = activeRegion ? (activeRegion.dataset.region || 'all') : 'all';
  window.__sportsScore = activeScore ? (activeScore.dataset.score || 'all') : 'all';
  window.__sportsSearch = sb ? (sb.value || '') : '';
  refreshVisibleCounts();
  applySportsFilters();
})();
</script>
...
def render_main(items: list[dict]) -> str:
    ...
    sections = {cat: sort_pickup([x for x in items if x.get("category") == cat])[:3] for cat in MAIN_SECTIONS}
    blocks = []
...
{filter_html}
<section class='panel'>
  <div class='section-head'><h2>🆕 NEW優先 注目3件 × 6種目</h2><span class='count-badge js-visible-count' data-target='.js-mini-scope .mini-item'>0件</span></div>
...
<section class='panel'>
  ...
  <div class='category-grid single js-mini-scope'>{''.join(mini_item(it) for it in focus_items)}</div>
</section>
<section>
  <div class='section-head'><h2>📰 最新記事</h2><span class='count-badge js-visible-count' data-target='.item-card'>0件</span></div>
...
def render_analysis(items: list[dict]) -> str:
    ...
    category_counts = {cat: sum(1 for x in items if x.get("category") == cat) for cat in MAIN_SECTIONS}
    region_counts = Counter(x.get("region", "Global") for x in items)
    topic_counts = Counter()
    for x in items:
        for t in x.get("themes", []):
            if t in SPORT_TOPIC_KEYWORDS:
                topic_counts[t] += 1
    ...
    rows2 = "".join(
        f"<div class='mini-item'><div style='font-weight:700'>{html.escape(name)}</div><div class='mini-meta'>{count}件</div></div>"
        for name, count in [("国内", region_counts.get("JP", 0)), ("海外", region_counts.get("Global", 0))]
    )
    rows3 = "".join(
        f"<div class='mini-item'><div style='font-weight:700'>{html.escape(k)}</div><div class='mini-meta'>{v}件</div></div>"
        for k, v in (topic_counts.most_common(6) or [("−", 0)])
    )
    rows4 = "".join(
    ...
    blocks_html = "".join([
        f"<section class='category-panel'><div class='section-head'><h2>種目別件数</h2></div><div class='mini-list'>{rows1}</div></section>",
        f"<section class='category-panel'><div class='section-head'><h2>地域別件数</h2></div><div class='mini-list'>{rows2}</div></section>",
        f"<section class='category-panel'><div class='section-head'><h2>トピック件数</h2></div><div class='mini-list'>{rows3}</div></section>",
        f"<section class='category-panel'><div class='section-head'><h2>直近7日 活発な種目</h2></div><div class='mini-list'>{rows4}</div></section>",
    ])
    top_cards = "".join(card_html(it) for it in sort_items(items)[:6])
    body = f"""
<div class='main'>
{filter_html}
<section class='panel'>
...
<section>
  <div class='section-head'><h2>⭐ 注目記事</h2><span class='count-badge'>スコア上位</span></div>
...
def fallback_items() -> list[dict]:
    now = datetime.now(JST)
    seeds = [
        ("野球", "野球トピックの取得結果がまだありません", "この版は起動確認を優先し、記事が0件でもHTMLを生成します。", "JP"),
        ("サッカー", "サッカートピックの取得結果がまだありません", "種目別ページ構成は watch_hub と同じ流儀です。", "JP"),
        ("バスケット", "バスケットトピックの取得結果がまだありません", "注目3件と最新記事の見せ方を watch_hub 準拠で揃えています。", "Global"),
        ("テニス", "テニストピックの取得結果がまだありません", "原文表示から翻訳ボタンで切り替える方式です。", "Global"),
        ("モータースポーツ", "モータースポーツトピックの取得結果がまだありません", "カードUIとフィルタ行は watch_hub 準拠です。", "Global"),
        ("格闘技", "格闘技トピックの取得結果がまだありません", "ソース差し替えでそのまま拡張できます。", "Global"),
    ]
    items = []
    for i, (cat, title, summary, region) in enumerate(seeds, start=1):
        items.append({
...
def main() -> None:
    ...
    pages = {
        ...
        "baseball.html": render_category_page(items, "野球", "baseball.html"),
        "soccer.html": render_category_page(items, "サッカー", "soccer.html"),
        "basketball.html": render_category_page(items, "バスケット", "basketball.html"),
        "tennis.html": render_category_page(items, "テニス", "tennis.html"),
        "motorsports.html": render_category_page(items, "モータースポーツ", "motorsports.html"),
        "combat.html": render_category_page(items, "格闘技", "combat.html"),
