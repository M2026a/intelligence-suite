...
USER_AGENT = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 {CONFIG.get('ua_suffix', 'EntertainmentPlus')}"
TIMEOUT    = 15
...
AI_DATA = ["movie", "film", "drama", "series", "trailer", "cast", "box office", "映画", "ドラマ", "シリーズ", "予告編", "キャスト", "興行収入"]
SECURITY_ALERT = ["music", "song", "album", "single", "artist", "tour", "concert", "音楽", "楽曲", "アルバム", "シングル", "アーティスト", "ツアー", "ライブ"]
CLOUD_PLATFORM = ["anime", "manga", "voice actor", "episode", "visual", "アニメ", "マンガ", "声優", "話", "ビジュアル"]
DEVELOPER_TOOLS = ["game", "gaming", "console", "steam", "playstation", "xbox", "nintendo", "ゲーム", "ゲーミング", "コンソール", "Steam", "PlayStation", "Xbox", "Nintendo"]
...
def read_js() -> str:
    return """
<script>
var READ_KEY = 'entertainment_plus_read';
function getReadSet() {
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    tab_cfg = [
        ("ai_data", "🎬 映画・ドラマ", "#7c4dff", "#1a1233"),
        ("security_alert", "🎵 音楽", "#ff5c5c", "#2f1111"),
        ("cloud_platform", "📚 アニメ・マンガ", "#00a8ff", "#0d1e35"),
        ("developer_tools", "🎮 ゲーム", "#20bf6b", "#0d2e1a"),
    ]
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx' data-fx='ai_data'>🎬 映画・ドラマ</button>
    <button class='flt-btn flt-fx' data-fx='security_alert'>🎵 音楽</button>
    <button class='flt-btn flt-fx' data-fx='cloud_platform'>📚 アニメ・マンガ</button>
    <button class='flt-btn flt-fx' data-fx='developer_tools'>🎮 ゲーム</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='ai_data'>🎬 映画・ドラマ</button>"
        "<button class='flt-btn flt-fx-t' data-fx='security_alert'>🎵 音楽</button>"
        "<button class='flt-btn flt-fx-t' data-fx='cloud_platform'>📚 アニメ・マンガ</button>"
        "<button class='flt-btn flt-fx-t' data-fx='developer_tools'>🎮 ゲーム</button>"
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='ai_data'>🎬 映画・ドラマ</button>
    <button class='flt-btn flt-fx-cb' data-fx='security_alert'>🎵 音楽</button>
    <button class='flt-btn flt-fx-cb' data-fx='cloud_platform'>📚 アニメ・マンガ</button>
    <button class='flt-btn flt-fx-cb' data-fx='developer_tools'>🎮 ゲーム</button>
  ...
  <div class='two-col'>
    ...
    <div>
      ...
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{fx_counts.get("ai_data",0)}</div><div style='color:var(--muted);font-size:11px'>🎬 映画・ドラマ</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{fx_counts.get("security_alert",0)}</div><div style='color:var(--muted);font-size:11px'>🎵 音楽</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{fx_counts.get("cloud_platform",0)}</div><div style='color:var(--muted);font-size:11px'>📚 アニメ・マンガ</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{fx_counts.get("developer_tools",0)}</div><div style='color:var(--muted);font-size:11px'>🎮 ゲーム</div></div>
      </div>
    </div>
  </div>
</div>"""
