...
DB_FILE              = OUTPUT_DIR / CONFIG.get("db_name", "dog_information_suite.db")
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 DogInformationSuite"
TIMEOUT    = 15
...
HEALTH_ALERT = [
    "health", "illness", "disease", "infection", "vaccine", "vet", "symptom", "parasite", "allergy",
    "heatstroke", "poison", "outbreak", "virus", "bacteria", "pet policy", "pet rules", "animal welfare law",
    "症状", "病気", "感染", "ワクチン", "獣医", "ペット可", "動物愛護", "マナー", "ルール", "狂犬病予防",
    "寄生虫", "アレルギー", "熱中症", "中毒", "注意喚起", "健康被害"
]
FOOD_RECALL = [
    "dog food", "pet food", "nutrition", "feeding", "treat", "kibble", "diet", "calorie", "recall",
    "contamination", "salmonella", "listeria", "dog goods", "pet supplies", "harness", "leash", "dog toy",
    "ドッグフード", "ペットフード", "栄養", "食事", "おやつ", "犬 グッズ", "ペット用品", "ハーネス", "リード",
    "カロリー", "リコール", "回収", "異物", "サルモネラ", "汚染", "犬 おもちゃ"
]
TRAINING_BEHAVIOR = [
    "training", "behavior", "obedience", "socialization", "barking", "aggression", "leash", "crate",
    "anxiety", "rehabilitation", "dog park", "pet friendly", "dog cafe", "travel with dog",
    "しつけ", "行動", "吠え", "攻撃性", "散歩", "クレート", "社会化",
    "分離不安", "問題行動", "ドッグラン", "ペット同伴", "犬 旅行", "犬 カフェ"
]
BREED_ADOPTION = [
    "breed", "puppy", "kennel", "working dog", "service dog", "adoption", "rescue", "shelter", "foster",
    "owner", "family dog", "adoption event", "rescue event", "adoption fair", "dog park", "pet friendly",
    "犬種", "子犬", "犬舎", "作業犬", "盲導犬", "譲渡", "保護犬", "シェルター",
    "里親", "飼い主", "家庭犬", "譲渡会", "保護活動", "里親会", "ドッグラン", "ペット同伴", "犬 カフェ"
]

# 参考度判定用
REFERENCE_WORDS = [
    "guide", "tips", "research", "study", "expert", "official", "guideline", "how to", "best practice",
    "ガイド", "解説", "研究", "調査", "専門家", "公式", "指針", "方法", "対策"
]
WARNING_WORDS = [
    "recall", "warning", "alert", "risk", "danger", "toxic", "outbreak", "died", "death", "ban",
    "リコール", "警告", "注意", "危険", "中毒", "流行", "死亡", "禁止", "回収"
]
...
def is_probable_duplicate_item(current: dict, previous: dict) -> bool:
    ...
    if soft_cur and soft_cur == soft_prev and similarity >= 0.84:
        return True
    if soft_cur and soft_prev and (soft_cur.startswith(soft_prev) or soft_prev.startswith(soft_cur)) and len(min(soft_cur, soft_prev, key=len)) >= 15:
        return True
    if similarity >= 0.91:
        return True
    return False
...
def infer_theme_from_source(source_name: str) -> list[str]:
    ...
    mapping = [
        (("健康", "病気", "vet", "health", "medical"), "健康・病気"),
        (("しつけ", "行動", "training", "behavior"), "しつけ・行動"),
        (("フード", "food", "nutrition", "recall", "栄養"), "フード・栄養"),
        (("犬種", "breed", "puppy", "子犬"), "犬種・子犬"),
        (("お出かけ", "スポット", "travel", "outing", "dog park", "pet friendly", "dog cafe", "旅行"), "お出かけ・スポット"),
        (("グッズ", "用品", "goods", "supplies", "harness", "toy"), "グッズ・用品"),
        (("譲渡会", "保護活動", "adoption event", "rescue event", "adoption fair", "里親会"), "譲渡会・保護活動"),
        (("制度", "マナー", "policy", "rules", "pet rules", "animal welfare", "狂犬病"), "ペット制度・マナー"),
        (("保護", "譲渡", "adoption", "rescue", "shelter", "里親"), "保護・暮らし"),
    ]
...
def infer_importance_from_source(source_name: str) -> str:
    src = text_lower(source_name)
    if any(k in src for k in ["健康", "病気", "vet", "health", "medical", "制度", "マナー", "policy", "rules", "狂犬病"]):
        return "health_alert"
    if any(k in src for k in ["フード", "food", "nutrition", "recall", "栄養", "グッズ", "用品", "goods", "supplies", "harness", "toy"]):
        return "food_recall"
    if any(k in src for k in ["しつけ", "行動", "training", "behavior", "お出かけ", "travel", "outing", "dog park", "pet friendly"]):
        return "training_behavior"
    if any(k in src for k in ["犬種", "breed", "puppy", "保護", "譲渡", "adoption", "rescue", "里親", "譲渡会", "保護活動"]):
    ...
    for entry in feed.entries:  # 上限なし・日付フィルターで制御
        ...
        try:
            if hasattr(entry, "published_parsed") and entry.published_parsed:
                pub_dt = datetime.fromtimestamp(calendar.timegm(entry.published_parsed[:6]), JST)
            elif hasattr(entry, "updated_parsed") and entry.updated_parsed:
                pub_dt = datetime.fromtimestamp(calendar.timegm(entry.updated_parsed[:6]), JST)
        except: pass
...
def collect_all(sources: list[dict], themes_cfg: dict) -> tuple[list[dict], int, int]:
    ...
    for item in all_items:
        ...
        if len(title_key) >= 25:
            candidate_keys.append(f"global_prefix::{title_key[:25]}")
        matched_prev = None
...
def read_js() -> str:
    return """
<script>
var READ_KEY = 'dog_information_suite_read';
function getReadSet() {
...
IMPORTANCE_MAP = {
    "health_alert": ("🩺 健康・病気", "tag-usd-strong"),
    "food_recall": ("🍖 フード・リコール", "tag-usd-weak"),
    "training_behavior": ("🎓 しつけ・行動", "tag-jpy-strong"),
    "breed_adoption": ("🏠 犬種・保護/暮らし", "tag-jpy-weak"),
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    tab_cfg = [
        ("health_alert", "🩺 健康・病気", "#4ea1ff", "#0d1e35"),
        ("food_recall",   "🍖 フード・リコール", "#e74c3c", "#2e0d0d"),
        ("training_behavior", "🎓 しつけ・行動",   "#2ecc71", "#0d2e1a"),
        ("breed_adoption",   "🏠 犬種・保護/暮らし",   "#f39c12", "#2e2400"),
    ]
...
<div class='stat-grid'>
  ...
  <div class='stat'><div class='icon'>🟢</div><div class='num' style='color:#4ea1ff'>{health_alert}</div><div class='label'>健康・症状</div></div>
  <div class='stat'><div class='icon'>🔴</div><div class='num' style='color:#e74c3c'>{food_recall}</div><div class='label'>フード・リコール</div></div>
  <div class='stat'><div class='icon'>🔵</div><div class='num' style='color:#2ecc71'>{training_behavior}</div><div class='label'>しつけ・行動</div></div>
  <div class='stat'><div class='icon'>🟡</div><div class='num' style='color:#f39c12'>{breed_adoption}</div><div class='label'>犬種・保護</div></div>
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx' data-fx='health_alert'>🩺 健康</button>
    <button class='flt-btn flt-fx' data-fx='food_recall'>🍖 フード</button>
    <button class='flt-btn flt-fx' data-fx='training_behavior'>🎓 しつけ</button>
    <button class='flt-btn flt-fx' data-fx='breed_adoption'>🏠 犬種・保護/暮らし</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='health_alert'>🩺 健康</button>"
        "<button class='flt-btn flt-fx-t' data-fx='food_recall'>🍖 フード</button>"
        "<button class='flt-btn flt-fx-t' data-fx='training_behavior'>🎓 しつけ</button>"
        "<button class='flt-btn flt-fx-t' data-fx='breed_adoption'>🏠 犬種・保護/暮らし</button>"
...
def render_sources(items: list[dict], themes_cfg: dict) -> str:
    stamp = datetime.now(JST).strftime("%Y-%m-%d %H:%M")
    source_meta = {
        "Google News JP - 犬 総合": {"icon": "🇯🇵", "desc": "国内犬ニュース総合"},
        "Google News JP - 犬 健康": {"icon": "🇯🇵", "desc": "国内の健康・病気情報"},
        "Google News JP - 犬 しつけ": {"icon": "🇯🇵", "desc": "国内のしつけ・行動情報"},
        "Google News JP - 犬 フード": {"icon": "🇯🇵", "desc": "国内のフード・栄養情報"},
        "Google News JP - 犬 お出かけ": {"icon": "🇯🇵", "desc": "国内のお出かけ・スポット情報"},
        "Google News JP - 犬 グッズ": {"icon": "🇯🇵", "desc": "国内のグッズ・用品情報"},
        "Google News JP - 譲渡会・保護": {"icon": "🇯🇵", "desc": "国内の譲渡会・保護活動情報"},
        "Google News JP - ペット制度・マナー": {"icon": "🇯🇵", "desc": "国内の制度・マナー情報"},
        "Google News EN - Dog Health": {"icon": "🌐", "desc": "海外の健康・病気情報"},
        "Google News EN - Dog Food Recall": {"icon": "🌐", "desc": "海外のフード・安全情報"},
        "Google News EN - Dog Training": {"icon": "🌐", "desc": "海外のしつけ・行動情報"},
        "Google News EN - Dog Outing Travel": {"icon": "🌐", "desc": "海外のお出かけ・旅行情報"},
        "Google News EN - Dog Goods": {"icon": "🌐", "desc": "海外のグッズ・用品情報"},
        "Google News EN - Dog Rescue Adoption": {"icon": "🌐", "desc": "海外の譲渡・保護情報"},
        "Google News - FDA Pet Food Recall": {"icon": "🏛", "desc": "米FDA関連の回収・安全情報"},
        "Google News - AVMA Dog": {"icon": "🏛", "desc": "米獣医師会関連情報"},
        "Google News - AKC Dog News": {"icon": "🏛", "desc": "AKC関連の犬ニュース"},
    }
...
<div class='panel'>
  <div class='filters'>
    ...
    <button class='flt-btn flt-cb-bank' data-bank='Google News JP - 犬 総合'>🇯🇵 国内総合</button>
    <button class='flt-btn flt-cb-bank' data-bank='Google News JP - 犬 健康'>🩺 国内健康</button>
    <button class='flt-btn flt-cb-bank' data-bank='Google News JP - 犬 お出かけ'>🌳 国内お出かけ</button>
    <button class='flt-btn flt-cb-bank' data-bank='Google News JP - 犬 グッズ'>🛍️ 国内グッズ</button>
    <button class='flt-btn flt-cb-bank' data-bank='Google News JP - 譲渡会・保護'>🤝 国内譲渡会</button>
    <button class='flt-btn flt-cb-bank' data-bank='Google News - FDA Pet Food Recall'>🏛 FDA</button>
    <button class='flt-btn flt-cb-bank' data-bank='Google News - AVMA Dog'>🏛 AVMA</button>
  </div>
  <div class='filters'>
    <span class='filter-label'>言語:</span>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='health_alert'>🩺 健康</button>
    <button class='flt-btn flt-fx-cb' data-fx='food_recall'>🍖 フード</button>
    <button class='flt-btn flt-fx-cb' data-fx='training_behavior'>🎓 しつけ</button>
    <button class='flt-btn flt-fx-cb' data-fx='breed_adoption'>🏠 犬種・保護/暮らし</button>
  ...
  <div class='two-col'>
    ...
    <div>
      ...
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{fx_counts.get("health_alert",0)}</div><div style='color:var(--muted);font-size:11px'>🩺 健康</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{fx_counts.get("food_recall",0)}</div><div style='color:var(--muted);font-size:11px'>🍖 フード</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{fx_counts.get("training_behavior",0)}</div><div style='color:var(--muted);font-size:11px'>🎓 しつけ</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{fx_counts.get("breed_adoption",0)}</div><div style='color:var(--muted);font-size:11px'>🏠 犬種・保護/暮らし</div></div>
      </div>
    </div>
  </div>
</div>"""
