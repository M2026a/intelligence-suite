...
MODEL_RELEASE = [
    "model", "llm", "foundation model", "release", "launch", "preview", "general availability",
    "GPT", "Gemini", "Claude", "Llama", "Mistral", "gpt-", "agent", "reasoning model",
    "モデル", "LLM", "基盤モデル", "公開", "リリース", "発表", "提供開始", "プレビュー", "一般提供",
    "生成AI", "エージェント", "推論モデル"
]
PRODUCT_UPDATE = [
    "product", "feature", "tool", "assistant", "copilot", "api", "sdk", "platform", "workspace",
    "search", "app", "service", "integration", "rollout", "pricing",
    "製品", "機能", "ツール", "アシスタント", "Copilot", "API", "SDK", "プラットフォーム", "サービス",
    "連携", "統合", "展開", "価格", "料金", "アップデート", "新機能"
]
RESEARCH_BREAKTHROUGH = [
    "research", "paper", "benchmark", "study", "dataset", "evaluation", "alignment", "safety research",
    "breakthrough", "arxiv", "conference", "nature", "science", "accuracy", "reasoning",
    "研究", "論文", "ベンチマーク", "調査", "データセット", "評価", "アラインメント", "安全性研究",
    "精度", "推論", "学会", "ブレークスルー"
]
POLICY_RISK = [
    "policy", "regulation", "law", "lawsuit", "copyright", "privacy", "antitrust", "security", "ethics",
    "governance", "compliance", "ban", "warning", "risk", "incident",
    "政策", "規制", "法", "訴訟", "著作権", "プライバシー", "独禁", "セキュリティ", "倫理",
    "ガバナンス", "コンプライアンス", "禁止", "警告", "リスク", "事故", "情報漏えい"
]


HIGH_SIGNAL = [
    "general availability", "enterprise", "deployment", "adoption", "governance", "compliance", "safety",
    "lawsuit", "copyright", "privacy", "regulation", "benchmark", "paper", "dataset",
    "nvidia", "gpu", "datacenter", "inference", "open source", "api", "pricing",
    "一般提供", "企業導入", "導入", "活用", "ガバナンス", "コンプライアンス", "安全性", "訴訟",
    "著作権", "プライバシー", "規制", "論文", "データセット", "半導体", "推論", "api", "価格", "料金"
]
LOW_SIGNAL = [
    "prompt", "prompting", "how to", "tips", "review", "opinion", "roundup", "top 10", "best ai",
    "fun", "creative", "meme", "image prompt", "hands-on", "summary", "digest",
    "プロンプト", "使い方", "コツ", "レビュー", "意見", "まとめ", "ランキング", "おすすめ", "遊び", "創作"
]
MODEL_NOISE = [
    "minor update", "small update", "bug fix", "availability in one region", "beta tweak",
    "軽微", "小規模", "細かな改善", "一部地域", "ベータ改善"
]
...
# 参考度判定用
REFERENCE_WORDS = [
    "guide", "research", "study", "paper", "benchmark", "official", "guideline", "best practice",
    "ガイド", "解説", "研究", "調査", "論文", "ベンチマーク", "公式", "指針", "方法", "対策"
]
WARNING_WORDS = [
    "warning", "alert", "risk", "danger", "ban", "lawsuit", "breach", "incident", "security",
    "警告", "注意", "危険", "禁止", "訴訟", "侵害", "事故", "情報漏えい", "セキュリティ"
]

# スコア加算用イベントキーワード
EVENT_KEYWORDS = {
    3: ["release", "launch", "lawsuit", "ban", "breach", "重大発表", "提供開始", "訴訟", "禁止", "情報漏えい"],
    2: ["research", "paper", "benchmark", "regulation", "guideline", "研究", "論文", "ベンチマーク", "規制", "指針"],
    1: ["feature", "api", "tool", "platform", "integration", "機能", "API", "ツール", "プラットフォーム", "連携"],
}
...
def is_new(pub_dt_iso: str) -> bool:
    ...
    try:
        ...
        return datetime.now(JST).astimezone(timezone.utc) - pub_dt <= timedelta(hours=NEW_HOURS)
    except: return False
...
def detect_fx_impact(title: str, summary: str) -> str:
    """重要タグを判定: model_release / product_update / research_breakthrough / policy_risk / neutral"""
    combined = text_lower(f"{title} {summary}")
    scores = {
        "model_release": sum(1 for w in MODEL_RELEASE if text_lower(w) in combined),
        "product_update": sum(1 for w in PRODUCT_UPDATE if text_lower(w) in combined),
        "research_breakthrough": sum(1 for w in RESEARCH_BREAKTHROUGH if text_lower(w) in combined),
        "policy_risk": sum(1 for w in POLICY_RISK if text_lower(w) in combined),
    }
...
def infer_theme_from_source(source_name: str) -> list[str]:
    ...
    mapping = [
        (("openai", "google ai", "microsoft ai", "anthropic", "hugging face", "developer", "newsroom"), "AI企業・開発"),
        (("model", "llm", "gpt", "gemini", "claude", "llama", "mistral", "decoder", "agent", "reasoning", "multimodal"), "生成AI・モデル"),
        (("product", "feature", "copilot", "api", "platform", "service", "ai+", "publickey", "workspace", "assistant", "tool", "sdk"), "AI製品・サービス"),
        (("research", "mit", "paper", "benchmark", "marktechpost", "study", "decoder"), "AI研究・論文"),
        (("policy", "regulation", "law", "copyright", "privacy", "governance", "bbc"), "AI政策・規制"),
        (("gpu", "chip", "semiconductor", "infrastructure", "datacenter", "pc watch", "xtech", "nvidia", "cuda", "accelerator"), "AI半導体・インフラ"),
        (("use case", "deployment", "adoption", "workflow", "business", "itmedia", "codezine", "automation", "enterprise", "implementation"), "AI活用・導入"),
        (("security", "ethics", "safety", "risk", "incident", "ars technica", "wired"), "AIセキュリティ・倫理"),
    ]
...
def infer_importance_from_source(source_name: str) -> str:
    src = text_lower(source_name)
    if any(k in src for k in ["openai", "google ai", "anthropic", "hugging face", "decoder", "model", "llm"]):
        return "model_release"
    if any(k in src for k in ["itmedia", "publickey", "codezine", "api", "copilot", "platform", "product"]):
        return "product_update"
    if any(k in src for k in ["mit", "marktechpost", "paper", "research", "benchmark"]):
        return "research_breakthrough"
    if any(k in src for k in ["bbc", "ars", "wired", "policy", "law", "regulation", "security"]):
        return "policy_risk"
    return "neutral"
...
def calc_score(item: dict) -> int:
    """AI+ 向けの価値判定。
    研究・政策・企業導入・基盤変化は厚く残し、ハウツーや軽いまとめ記事は抑える。
    """
    ...
    high_signal_hits = count_keyword_hits(combined, HIGH_SIGNAL)
    low_signal_hits = count_keyword_hits(combined, LOW_SIGNAL)
    model_noise_hits = count_keyword_hits(combined, MODEL_NOISE)
    ...
    if low_signal_hits >= 2:
        score -= 2
    elif low_signal_hits == 1:
        score -= 1

    if fx_impact == "policy_risk":
        score += 1
    elif fx_impact == "research_breakthrough":
        score += 1
    elif fx_impact == "product_update":
        if any(k in combined for k in ["enterprise", "deployment", "pricing", "api", "copilot", "企業導入", "導入", "料金"]):
            score += 1
    elif fx_impact == "model_release":
        if model_noise_hits >= 1 and high_signal_hits == 0:
            score -= 1
        if any(k in combined for k in ["open source", "reasoning", "multimodal", "context window", "benchmark", "安全性", "推論", "マルチモーダル", "ベンチマーク"]):
            score += 1
...
def classify_themes(title: str, summary: str, lang: str, themes_cfg: dict) -> list[str]:
    ...
    for theme_key, theme_data in themes_cfg["themes"].items():
        keywords = list(theme_data.get("keywords_en", [])) + list(theme_data.get("keywords_ja", []))
        for kw in keywords:
            kw_norm = text_lower(kw)
...
class _AnthropicParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if href and re.match(r'^/news/[a-z0-9][a-z0-9-]+', href):
            self._in_link = True
    ...
    def handle_endtag(self, tag):
        if tag == "a" and self._in_link:
            ...
            if title and len(title) > 8 and self._current_href:
                if self._current_href not in {a["link"] for a in self.articles}:
...
def fetch_anthropic_newsroom(themes_cfg: dict) -> tuple[list[dict], bool]:
    ...
    try:
        ...
        found_dates = date_pattern.findall(html_text)

        items = []
        now_utc = datetime.now(JST).astimezone(timezone.utc)
        for i, art in enumerate(parser.articles[:30]):
            ...
            if i < len(found_dates):
                raw_date = found_dates[i][0] or found_dates[i][1] or found_dates[i][2]
                if raw_date:
                    try:
                        dt = datetime.fromisoformat(raw_date.replace('Z', '+00:00')[:25])
            ...
            themes = classify_themes(title, "", "en", themes_cfg)
            if themes == ["その他"]:
                themes = ["AI企業・開発"]
            item = {
...
def fetch_source(source: dict, themes_cfg: dict) -> tuple[list[dict], bool]:
    ...
    for entry in feed.entries:  # 上限なし・日付フィルターで制御
        ...
        if pub_dt:
            if datetime.now(JST).astimezone(timezone.utc) - pub_dt > timedelta(days=HISTORY_DAYS):
                continue
...
def collect_all(sources: list[dict], themes_cfg: dict) -> tuple[list[dict], int, int]:
    ...
    anthropic_items, anthropic_success = fetch_anthropic_newsroom(themes_cfg)
...
IMPORTANCE_MAP = {
    "model_release": ("🤖 モデル・発表", "tag-usd-strong"),
    "product_update": ("🧩 製品・機能", "tag-usd-weak"),
    "research_breakthrough": ("🧪 研究・論文", "tag-jpy-strong"),
    "policy_risk": ("⚖️ 政策・規制", "tag-jpy-weak"),
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    cats = {
        "model_release": [x for x in items if x.get("fx_impact") == "model_release"],
        "product_update":   [x for x in items if x.get("fx_impact") == "product_update"],
        "research_breakthrough": [x for x in items if x.get("fx_impact") == "research_breakthrough"],
        "policy_risk":   [x for x in items if x.get("fx_impact") == "policy_risk"],
    }
    neutral_count = sum(1 for x in items if x.get("fx_impact") == "neutral")

    tab_cfg = [
        ("model_release", "🤖 モデル・発表", "#4ea1ff", "#0d1e35"),
        ("product_update",   "🧩 製品・機能", "#e74c3c", "#2e0d0d"),
        ("research_breakthrough", "🧪 研究・論文",   "#2ecc71", "#0d2e1a"),
        ("policy_risk",   "⚖️ 政策・規制",   "#f39c12", "#2e2400"),
    ]
...
<div class='fx-summary-grid'>
  <div class='fx-summary-card fx-usd-strong'>
    <div class='fx-num' style='color:#4ea1ff'>{len(cats["model_release"] )}</div>
    <div class='fx-label'>🤖 モデル・発表</div>
    <div class='fx-desc'>Models / Releases</div>
  </div>
  <div class='fx-summary-card fx-usd-weak'>
    <div class='fx-num' style='color:#e74c3c'>{len(cats["product_update"] )}</div>
    <div class='fx-label'>🧩 製品・機能</div>
    <div class='fx-desc'>Products / Features</div>
  </div>
  <div class='fx-summary-card fx-jpy-strong'>
    <div class='fx-num' style='color:#2ecc71'>{len(cats["research_breakthrough"] )}</div>
    <div class='fx-label'>🧪 研究・論文</div>
    <div class='fx-desc'>Research / Papers</div>
  </div>
  <div class='fx-summary-card fx-jpy-weak'>
    <div class='fx-num' style='color:#f39c12'>{len(cats["policy_risk"] )}</div>
    <div class='fx-label'>⚖️ 政策・規制</div>
    <div class='fx-desc'>Policy / Regulation</div>
  </div>
...
def render_index(items: list[dict], themes_cfg: dict) -> str:
    ...
    model_release = sum(1 for x in items if x.get("fx_impact") == "model_release")
    product_update   = sum(1 for x in items if x.get("fx_impact") == "product_update")
    research_breakthrough = sum(1 for x in items if x.get("fx_impact") == "research_breakthrough")
    policy_risk   = sum(1 for x in items if x.get("fx_impact") == "policy_risk")
...
<div class='stat-grid'>
  ...
  <div class='stat'><div class='icon'>🟢</div><div class='num' style='color:#4ea1ff'>{model_release}</div><div class='label'>モデル・発表</div></div>
  <div class='stat'><div class='icon'>🔴</div><div class='num' style='color:#e74c3c'>{product_update}</div><div class='label'>製品・機能</div></div>
  <div class='stat'><div class='icon'>🔵</div><div class='num' style='color:#2ecc71'>{research_breakthrough}</div><div class='label'>研究・論文</div></div>
  <div class='stat'><div class='icon'>🟡</div><div class='num' style='color:#f39c12'>{policy_risk}</div><div class='label'>政策・規制</div></div>
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx' data-fx='model_release'>🤖 モデル</button>
    <button class='flt-btn flt-fx' data-fx='product_update'>🧩 製品</button>
    <button class='flt-btn flt-fx' data-fx='research_breakthrough'>🧪 研究</button>
    <button class='flt-btn flt-fx' data-fx='policy_risk'>⚖️ 政策</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='model_release'>🤖 モデル</button>"
        "<button class='flt-btn flt-fx-t' data-fx='product_update'>🧩 製品</button>"
        "<button class='flt-btn flt-fx-t' data-fx='research_breakthrough'>🧪 研究</button>"
        "<button class='flt-btn flt-fx-t' data-fx='policy_risk'>⚖️ 政策</button>"
...
def render_sources(items: list[dict], themes_cfg: dict) -> str:
    stamp = datetime.now(JST).strftime("%Y-%m-%d %H:%M")
    source_meta = {
        "OpenAI Blog": {"icon": "🟢", "desc": "OpenAI公式ブログ"},
        "Google AI Blog": {"icon": "🔵", "desc": "Google AI公式ブログ"},
        "Microsoft AI Blog": {"icon": "🟦", "desc": "Microsoft AI公式ブログ"},
        "Hugging Face Blog": {"icon": "🤗", "desc": "Hugging Face公式ブログ"},
        "Anthropic Newsroom": {"icon": "🧠", "desc": "Anthropic公式ニュースルーム"},
        "TechCrunch AI": {"icon": "📰", "desc": "AI業界ニュース"},
        "The Verge AI": {"icon": "📰", "desc": "テクノロジーニュース"},
        "MIT Tech Review": {"icon": "🎓", "desc": "研究・解説"},
        "Wired AI": {"icon": "📘", "desc": "AIトレンド・論考"},
        "Ars Technica AI": {"icon": "🧪", "desc": "技術解説・検証"},
        "The Decoder": {"icon": "🤖", "desc": "生成AI・モデル速報"},
        "MarkTechPost": {"icon": "📄", "desc": "研究・論文紹介"},
        "BBC Technology": {"icon": "🌍", "desc": "国際テクノロジー報道"},
        "ITmedia AI+": {"icon": "🇯🇵", "desc": "国内AIニュース"},
        "Publickey": {"icon": "🇯🇵", "desc": "国内技術解説"},
        "CodeZine": {"icon": "🇯🇵", "desc": "開発者向け記事"},
        "ASCII.jp": {"icon": "🇯🇵", "desc": "国内テックニュース"},
        "日経XTECH": {"icon": "🇯🇵", "desc": "IT・DX・技術報道"},
        "Gigazine": {"icon": "🇯🇵", "desc": "国内テクノロジー記事"},
        "Impress Watch": {"icon": "🇯🇵", "desc": "国内テック速報"},
        "PC Watch": {"icon": "🇯🇵", "desc": "PC・半導体・AIインフラ"},
    }
...
<div class='panel'>
  <div class='filters'>
    ...
    <button class='flt-btn flt-cb-bank' data-bank='OpenAI Blog'>🟢 OpenAI</button>
    <button class='flt-btn flt-cb-bank' data-bank='Google AI Blog'>🔵 Google</button>
    <button class='flt-btn flt-cb-bank' data-bank='Anthropic Newsroom'>🧠 Anthropic</button>
    <button class='flt-btn flt-cb-bank' data-bank='TechCrunch AI'>📰 TechCrunch</button>
    <button class='flt-btn flt-cb-bank' data-bank='ITmedia AI+'>🇯🇵 ITmedia</button>
    <button class='flt-btn flt-cb-bank' data-bank='Publickey'>🇯🇵 Publickey</button>
    <button class='flt-btn flt-cb-bank' data-bank='日経XTECH'>🇯🇵 日経XTECH</button>
  </div>
  <div class='filters'>
    <span class='filter-label'>言語:</span>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='model_release'>🤖 モデル</button>
    <button class='flt-btn flt-fx-cb' data-fx='product_update'>🧩 製品</button>
    <button class='flt-btn flt-fx-cb' data-fx='research_breakthrough'>🧪 研究</button>
    <button class='flt-btn flt-fx-cb' data-fx='policy_risk'>⚖️ 政策</button>
  ...
  <div class='two-col'>
    ...
    <div>
      ...
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{fx_counts.get("model_release",0)}</div><div style='color:var(--muted);font-size:11px'>🤖 モデル</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{fx_counts.get("product_update",0)}</div><div style='color:var(--muted);font-size:11px'>🧩 製品</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{fx_counts.get("research_breakthrough",0)}</div><div style='color:var(--muted);font-size:11px'>🧪 研究</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{fx_counts.get("policy_risk",0)}</div><div style='color:var(--muted);font-size:11px'>⚖️ 政策・規制</div></div>
      </div>
    </div>
  </div>
</div>"""
