...
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 ItPlus"
TIMEOUT    = 15
...
AI_DATA = [
    ...
    "foundation model", "agent", "copilot", "chatgpt", "prompt", "inference",
    "AI", "人工知能", "機械学習", "生成AI", "LLM", "大規模言語モデル", "基盤モデル",
    "エージェント", "プロンプト", "推論"
]
SECURITY_ALERT = [
    "security", "cybersecurity", "vulnerability", "cve", "ransomware", "malware", "phishing",
    "breach", "zero-day", "exploit", "attack", "patch", "incident",
    "セキュリティ", "脆弱性", "CVE", "ランサムウェア", "マルウェア", "フィッシング",
    "侵害", "ゼロデイ", "攻撃", "パッチ", "インシデント", "情報漏えい"
]
CLOUD_PLATFORM = [
    "cloud", "aws", "azure", "google cloud", "gcp", "kubernetes", "container", "serverless",
    "infrastructure", "network", "datacenter", "edge", "virtualization", "vmware",
    "クラウド", "AWS", "Azure", "Google Cloud", "GCP", "Kubernetes", "コンテナ",
    "サーバレス", "インフラ", "ネットワーク", "データセンター", "仮想化", "VMware"
]
DEVELOPER_TOOLS = [
    "developer", "devops", "ci/cd", "github", "gitlab", "api", "sdk", "framework",
    "programming", "python", "java", "javascript", "release", "open source",
    "開発", "開発者", "DevOps", "CI/CD", "GitHub", "GitLab", "API", "SDK",
    "フレームワーク", "プログラミング", "Python", "Java", "JavaScript", "オープンソース", "リリース"
]


AI_HIGH_VALUE = [
    "copilot for microsoft 365", "microsoft 365 copilot", "github copilot", "copilot studio",
    "enterprise", "business", "workflow", "productivity", "deployment", "rollout", "integration",
    "governance", "compliance", "security", "privacy", "data protection", "license", "licensing", "pricing",
    "aws bedrock", "azure ai", "azure openai", "google workspace", "salesforce", "servicenow",
    "agent", "agents", "rag", "ai search", "ai pc", "semiconductor", "accelerator", "gpu",
    "copilot", "導入", "展開", "企業導入", "業務", "業務利用", "業務活用", "運用", "実装", "統合",
    "連携", "ガバナンス", "コンプライアンス", "セキュリティ", "プライバシー", "保護", "ライセンス", "価格",
    "料金", "bedrock", "azure ai", "azure openai", "ワークフロー", "生産性", "rag", "ai検索", "ai pc"
]
AI_LOW_VALUE = [
    "benchmark", "leaderboard", "demo", "prompt", "prompting", "prompt engineering",
    "creative", "image generator", "chatbot", "fun", "top 10", "roundup", "review",
    "opinion", "commentary", "hands-on", "vs.", "versus", "comparison", "how to use chatgpt",
    "ベンチマーク", "ランキング", "デモ", "プロンプト", "画像生成", "チャットボット", "特集", "まとめ",
    "レビュー", "比較", "使ってみた", "入門", "やってみた", "コツ", "テクニック"
]
NON_AI_STRATEGIC = [
    "zero-day", "cve", "ransomware", "patch", "outage", "migration", "kubernetes", "vmware",
    "oracle", "database", "postgresql", "network", "observability", "supply chain", "firmware",
    "ゼロデイ", "脆弱性", "ランサムウェア", "パッチ", "障害", "移行", "データベース", "ネットワーク",
    "監視", "可観測性", "サプライチェーン", "ファームウェア"
]
...
def infer_theme_from_source(source_name: str) -> list[str]:
    ...
    mapping = [
        (("techcrunch", "zdnet", "register"), "企業・業界動向"),
        (("aws",), "クラウド・インフラ"),
        (("bleepingcomputer", "security"), "セキュリティ"),
        (("infoq", "github", "gitlab", "developer", "devops"), "開発ツール・DevOps"),
        (("ars", "semiconductor", "chip", "hardware", "pc"), "ハードウェア・半導体"),
        (("ai", "machine learning", "copilot", "llm"), "AI・機械学習"),
        (("data", "database", "analytics"), "データ・分析"),
        (("industry", "market", "business", "enterprise"), "企業・業界動向"),
    ]
...
def calc_score(item: dict) -> int:
    """IT+ 向けの価値判定。
    AI記事は無条件加点せず、業務影響・導入影響・統制影響があるものだけ強く残す。
    逆に比較記事・ハウツー・デモ系は減点する。
    """
...
<div class='wrap'>
  <div class='top'>
    <div class='title'>
      <h1>🖥️ {html.escape(APP_NAME)} <span style='font-weight:400;color:var(--muted);font-size:16px'>｜ {html.escape(CONFIG.get("subtitle", ""))}</span></h1>
...
IMPORTANCE_MAP = {
    "ai_data": ("🤖 AI・データ", "tag-usd-strong"),
    "security_alert": ("🔐 セキュリティ", "tag-usd-weak"),
    "cloud_platform": ("☁️ クラウド・基盤", "tag-jpy-strong"),
    "developer_tools": ("🛠️ 開発ツール", "tag-jpy-weak"),
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    tab_cfg = [
        ("ai_data", "🤖 AI・データ", "#7c4dff", "#1a1233"),
        ("security_alert", "🔐 セキュリティ", "#ff5c5c", "#2f1111"),
        ("cloud_platform", "☁️ クラウド・基盤", "#00a8ff", "#0d1e35"),
        ("developer_tools", "🛠️ 開発ツール", "#20bf6b", "#0d2e1a"),
    ]
...
<div class='stat-grid'>
  ...
  <div class='stat'><div class='icon'>🤖</div><div class='num' style='color:#7c4dff'>{ai_data}</div><div class='label'>AI・データ</div></div>
  <div class='stat'><div class='icon'>🔐</div><div class='num' style='color:#ff5c5c'>{security_alert}</div><div class='label'>セキュリティ</div></div>
  <div class='stat'><div class='icon'>☁️</div><div class='num' style='color:#00a8ff'>{cloud_platform}</div><div class='label'>クラウド・基盤</div></div>
  <div class='stat'><div class='icon'>🛠️</div><div class='num' style='color:#20bf6b'>{developer_tools}</div><div class='label'>開発ツール</div></div>
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx' data-fx='security_alert'>🔐 セキュリティ</button>
    <button class='flt-btn flt-fx' data-fx='cloud_platform'>☁️ クラウド</button>
    <button class='flt-btn flt-fx' data-fx='developer_tools'>🛠️ 開発ツール</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='security_alert'>🔐 セキュリティ</button>"
        "<button class='flt-btn flt-fx-t' data-fx='cloud_platform'>☁️ クラウド</button>"
        "<button class='flt-btn flt-fx-t' data-fx='developer_tools'>🛠️ 開発ツール</button>"
...
def render_sources(items: list[dict], themes_cfg: dict) -> str:
    stamp = datetime.now(JST).strftime("%Y-%m-%d %H:%M")
    source_meta = {
        "TechCrunch": {"icon": "🌐", "desc": "海外ITニュース総合"},
        "Ars Technica": {"icon": "💻", "desc": "技術解説・ハードウェア"},
        "InfoQ": {"icon": "🛠️", "desc": "開発・アーキテクチャ"},
        "The Register": {"icon": "🏢", "desc": "インフラ・業界動向"},
        "BleepingComputer": {"icon": "🔐", "desc": "セキュリティ・脅威情報"},
        "ZDNET Japan": {"icon": "🇯🇵", "desc": "国内ITニュース総合"},
        "ITmedia NEWS 国内": {"icon": "🇯🇵", "desc": "国内ITニュース"},
        "ITmedia NEWS セキュリティ": {"icon": "🔐", "desc": "国内セキュリティ情報"},
        "ITmedia NEWS 企業・業界動向": {"icon": "🏢", "desc": "国内企業・業界動向"},
        "ITmedia AI＋": {"icon": "🤖", "desc": "国内AI情報"},
        "AWS News Blog": {"icon": "☁️", "desc": "AWS公式アップデート"},
    }
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='security_alert'>🔐 セキュリティ</button>
    <button class='flt-btn flt-fx-cb' data-fx='cloud_platform'>☁️ クラウド</button>
    <button class='flt-btn flt-fx-cb' data-fx='developer_tools'>🛠️ 開発ツール</button>
  ...
  <div class='two-col'>
    ...
    <div>
      ...
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{fx_counts.get("ai_data",0)}</div><div style='color:var(--muted);font-size:11px'>🤖 AI・データ</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{fx_counts.get("security_alert",0)}</div><div style='color:var(--muted);font-size:11px'>🔐 セキュリティ</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{fx_counts.get("cloud_platform",0)}</div><div style='color:var(--muted);font-size:11px'>☁️ クラウド</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{fx_counts.get("developer_tools",0)}</div><div style='color:var(--muted);font-size:11px'>🛠️ 開発ツール</div></div>
      </div>
    </div>
  </div>
</div>"""
