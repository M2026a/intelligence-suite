...
USER_AGENT = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 {CONFIG.get('ua_suffix', 'SportsPlus')}"
TIMEOUT    = 15
...
AI_DATA = ["match", "game", "result", "score", "win", "loss", "draw", "playoff", "final", "試合", "結果", "得点", "勝利", "敗戦", "引き分け", "プレーオフ", "決勝"]
SECURITY_ALERT = ["transfer", "contract", "signing", "deal", "trade", "loan", "extension", "移籍", "契約", "加入", "退団", "トレード", "レンタル", "延長"]
CLOUD_PLATFORM = ["player", "team", "coach", "manager", "captain", "roster", "lineup", "選手", "チーム", "監督", "コーチ", "主将", "ロスター", "スタメン"]
DEVELOPER_TOOLS = ["injury", "injured", "absence", "out", "return", "rehab", "sidelined", "故障", "負傷", "欠場", "離脱", "復帰", "リハビリ"]
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    tab_cfg = [
        ("ai_data", "🏟️ 試合・結果", "#7c4dff", "#1a1233"),
        ("security_alert", "🔁 移籍・契約", "#ff5c5c", "#2f1111"),
        ("cloud_platform", "⭐ 選手・チーム", "#00a8ff", "#0d1e35"),
        ("developer_tools", "🚑 故障・欠場", "#20bf6b", "#0d2e1a"),
    ]
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx' data-fx='ai_data'>🏟️ 試合・結果</button>
    <button class='flt-btn flt-fx' data-fx='security_alert'>🔁 移籍・契約</button>
    <button class='flt-btn flt-fx' data-fx='cloud_platform'>⭐ 選手・チーム</button>
    <button class='flt-btn flt-fx' data-fx='developer_tools'>🚑 故障・欠場</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='ai_data'>🏟️ 試合・結果</button>"
        "<button class='flt-btn flt-fx-t' data-fx='security_alert'>🔁 移籍・契約</button>"
        "<button class='flt-btn flt-fx-t' data-fx='cloud_platform'>⭐ 選手・チーム</button>"
        "<button class='flt-btn flt-fx-t' data-fx='developer_tools'>🚑 故障・欠場</button>"
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='ai_data'>🏟️ 試合・結果</button>
    <button class='flt-btn flt-fx-cb' data-fx='security_alert'>🔁 移籍・契約</button>
    <button class='flt-btn flt-fx-cb' data-fx='cloud_platform'>⭐ 選手・チーム</button>
    <button class='flt-btn flt-fx-cb' data-fx='developer_tools'>🚑 故障・欠場</button>
  ...
  <div class='two-col'>
    ...
    <div>
      ...
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{fx_counts.get("ai_data",0)}</div><div style='color:var(--muted);font-size:11px'>🏟️ 試合・結果</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{fx_counts.get("security_alert",0)}</div><div style='color:var(--muted);font-size:11px'>🔁 移籍・契約</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{fx_counts.get("cloud_platform",0)}</div><div style='color:var(--muted);font-size:11px'>⭐ 選手・チーム</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{fx_counts.get("developer_tools",0)}</div><div style='color:var(--muted);font-size:11px'>🚑 故障・欠場</div></div>
      </div>
    </div>
  </div>
</div>"""
