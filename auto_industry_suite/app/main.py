...
DB_FILE = OUTPUT / CONFIG.get("db_name", "auto_industry_suite.db")
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 AutoIndustrySuite"
TIMEOUT = 15
...
HEALTH_ALERT = [
    "battery", "ev", "electric vehicle", "electrification", "charging", "plug-in", "hybrid", "phev", "bev",
    "電気自動車", "EV", "BEV", "PHEV", "ハイブリッド", "電動化", "充電", "バッテリー", "電池", "急速充電",
    "e-axle", "solid-state battery", "LFP", "NCM", "range", "charging network"
]
FOOD_RECALL = [
    "production", "factory", "plant", "assembly", "manufacturing", "capacity", "supplier", "supply chain", "procurement",
    "semiconductor", "chip shortage", "logistics", "inventory", "shutdown", "line-off", "生産", "工場", "組立", "製造",
    "供給網", "サプライチェーン", "調達", "半導体", "在庫", "物流", "稼働", "停止", "部品", "供給"
]
TRAINING_BEHAVIOR = [
    "regulation", "policy", "tariff", "subsidy", "incentive", "emissions", "safety", "recall", "compliance",
    "antitrust", "government", "rule", "law", "規制", "政策", "関税", "補助金", "優遇", "排出", "安全", "リコール",
    "法規", "法令", "認証", "行政", "基準", "義務化"
]
BREED_ADOPTION = [
    "earnings", "revenue", "profit", "operating income", "forecast", "sales", "deliveries", "market share",
    "investment", "partnership", "joint venture", "merger", "acquisition", "guidance", "業績", "売上", "利益", "営業利益",
    "見通し", "販売", "市場", "シェア", "投資", "提携", "合弁", "買収", "決算"
]

# 参考度判定用
REFERENCE_WORDS = [
    "official", "guideline", "policy", "earnings", "production", "investment", "sales", "forecast",
    "公式", "方針", "政策", "決算", "生産", "投資", "販売", "見通し", "発表", "説明"
]
WARNING_WORDS = [
    "recall", "warning", "risk", "tariff", "shortage", "shutdown", "lawsuit", "delay", "ban",
    "リコール", "警告", "リスク", "関税", "不足", "停止", "訴訟", "遅延", "禁止", "下方修正"
]

# スコア加算用イベントキーワード
EVENT_KEYWORDS = {
    3: ["recall", "tariff", "shutdown", "shortage", "lawsuit", "リコール", "関税", "生産停止", "不足", "訴訟"],
    2: ["battery", "ev", "investment", "subsidy", "regulation", "バッテリー", "EV", "投資", "補助金", "規制"],
    1: ["sales", "market", "production", "launch", "partnership", "販売", "市場", "生産", "発売", "提携"],
}
...
def format_jst(dt: datetime | None, fmt: str = "%Y-%m-%d %H:%M") -> str:
    dt = ensure_jst(dt)
...
def source_group(name: str, region: str = "", official: bool = False, lang: str = "") -> str:
    src = text_lower(name)
    region = text_lower(region)
    lang = text_lower(lang)

    if src in ["response", "car watch"]:
        return "jp_media"
    if "toyota" in src or "honda" in src or "nissan" in src:
        return "official_oem"
    if "electrek" in src or "insideevs" in src:
        return "ev_media"
    if "reuters" in src:
        return "industry_media"
    if official:
        return "official_oem"
    if region == "japan" or lang == "ja":
        return "jp_media"
    return "global_media"
...
def is_new(pub_dt_iso: str) -> bool:
    ...
    try:
        ...
        pub_dt = ensure_jst(pub_dt)
        return now_jst() - pub_dt <= timedelta(hours=NEW_HOURS)
    except: return False
...
def detect_fx_impact(title: str, summary: str) -> str:
    """重要タグを判定: ev_shift / supply_chain / regulation / earnings_market / neutral"""
    combined = text_lower(f"{title} {summary}")
    scores = {
        "ev_shift": sum(1 for w in HEALTH_ALERT if text_lower(w) in combined),
        "supply_chain": sum(1 for w in FOOD_RECALL if text_lower(w) in combined),
        "regulation": sum(1 for w in TRAINING_BEHAVIOR if text_lower(w) in combined),
        "earnings_market": sum(1 for w in BREED_ADOPTION if text_lower(w) in combined),
    }
...
def infer_theme_from_source(source_name: str) -> list[str]:
    ...
    mapping = [
        (("ev", "electric", "electr", "battery", "charging", "toyota global newsroom", "honda global newsroom"), "EV・電動化"),
        (("production", "factory", "plant", "supply", "car watch"), "生産・供給網"),
        (("sales", "market", "response"), "販売・市場"),
        (("policy", "regulation", "reuters", "government", "tariff"), "政策・規制"),
        (("nissan", "software", "sdv", "autonomous", "adas"), "自動運転・SDV"),
        (("earnings", "finance", "profit"), "業績・財務"),
    ]
...
def infer_importance_from_source(source_name: str) -> str:
    src = text_lower(source_name)
    if any(k in src for k in ["ev", "electric", "battery", "charging", "electr"]):
        return "ev_shift"
    if any(k in src for k in ["production", "factory", "plant", "supply", "car watch"]):
        return "supply_chain"
    if any(k in src for k in ["policy", "regulation", "tariff", "reuters", "government"]):
        return "regulation"
    if any(k in src for k in ["sales", "market", "earnings", "finance", "profit", "response"]):
        return "earnings_market"
    return "neutral"
...
def compute_score(source_name: str, title: str, summary: str, official: bool) -> int:
    """互換用スコア関数。旧呼び出し形を現在の calc_score に橋渡しする。"""
    item = {
        "source": source_name,
        "title": title,
        "summary": summary,
        "official": official,
        "themes": infer_theme_from_source(source_name),
        "sentiment": detect_sentiment(title, summary),
    }
    return calc_score(item)
...
def db_save_items(conn: sqlite3.Connection, items: list[dict]) -> None:
    ...
    rows = [(
        ...
        item.get("title", ""), item.get("title_ja",""), item.get("summary", ""), item.get("summary_ja",""),
        item.get("link") or item.get("url", ""), item.get("published", ""), item.get("pub_dt", ""),
...
def fetch_source(source: dict, themes_cfg: dict) -> tuple[list[dict], bool]:
    ...
    max_items = int(source.get("max_items", CONFIG.get("max_items_per_source", 10)))
    source_type = str(source.get("type", "rss")).lower().strip()

    log(f"  取得中: {name}")

    def build_item(title: str, summary: str, link: str, pub_str: str = "", pub_dt_iso: str = "") -> dict:
        ...
        item["score"] = compute_score(name, title, summary, bool(source.get("official", False)))
        return item

    try:
        items = []
        if source_type == "html":
            headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AutoIndustrySuite"}
            resp = requests.get(url, headers=headers, timeout=20)
            if resp.status_code >= 400:
                log(f"  ⚠ {name}: HTTP {resp.status_code} for {url}")
                return [], True
            soup = BeautifulSoup(resp.text, "html.parser")
            base_url = source.get("base_url", url)
            item_selector = source.get("item_selector", "a")
            link_contains = source.get("link_contains", "")
            summary_selector = source.get("summary_selector", "")
            min_title_len = int(source.get("min_title_len", 12))
            seen_links = set()
            ...
            for node in nodes:
                ...
                if summary_selector:
                    try:
                        parent = node.parent
                        s_node = parent.select_one(summary_selector) if parent else None
                        if s_node:
                            summary = clean_text(s_node.get_text(" ", strip=True)) or title
                    except Exception:
                        pass
                if is_image_or_gallery_item(title, summary, link):
                    continue
                seen_links.add(link)
                items.append(build_item(title, summary, link))
            ...
            for entry in entries:
                ...
                if not pub_dt:
                    raw_pub = getattr(entry, "published", "") or getattr(entry, "updated", "") or ""
                    pub_dt = parse_datetime_text(raw_pub)
                pub_str = format_jst(pub_dt)
                items.append(build_item(title, summary, link, pub_str, pub_dt.isoformat() if pub_dt else ""))
                if len(items) >= max_items:
                    break
...
def read_js() -> str:
    return """
<script>
var READ_KEY = 'auto_industry_suite_read';
function getReadSet() {
...
IMPORTANCE_MAP = {
    "ev_shift": ("⚡ EV・電動化", "tag-usd-strong"),
    "supply_chain": ("🏭 生産・供給網", "tag-usd-weak"),
    "regulation": ("⚖ 政策・規制", "tag-jpy-strong"),
    "earnings_market": ("💹 業績・市場", "tag-jpy-weak"),
...
def card_html(item: dict, themes_cfg: dict) -> str:
    ...
    src_group   = html.escape(source_group(item.get("source", ""), region, item.get("official", False), lang))
...
<article class='card item-card'
  ...
  data-score='{score}'
  data-source='{source}'
  data-sourcegroup='{src_group}'
  data-region='{html.escape(region)}'
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    cats = {
        "ev_shift": [x for x in items if x.get("fx_impact") == "ev_shift"],
        "supply_chain":   [x for x in items if x.get("fx_impact") == "supply_chain"],
        "regulation": [x for x in items if x.get("fx_impact") == "regulation"],
        "earnings_market":   [x for x in items if x.get("fx_impact") == "earnings_market"],
    }
    neutral_count = sum(1 for x in items if x.get("fx_impact") == "neutral")

    tab_cfg = [
        ("ev_shift", "⚡ EV・電動化", "#4ea1ff", "#0d1e35"),
        ("supply_chain",   "🏭 生産・供給網", "#e74c3c", "#2e0d0d"),
        ("regulation", "⚖ 政策・規制",   "#2ecc71", "#0d2e1a"),
        ("earnings_market",   "💹 業績・市場",   "#f39c12", "#2e2400"),
    ]
...
<div class='fx-summary-grid'>
  <div class='fx-summary-card fx-usd-strong'>
    <div class='fx-num' style='color:#4ea1ff'>{len(cats["ev_shift"] )}</div>
    <div class='fx-label'>⚡ EV・電動化</div>
    <div class='fx-desc'>EV / Electrification</div>
  </div>
  <div class='fx-summary-card fx-usd-weak'>
    <div class='fx-num' style='color:#e74c3c'>{len(cats["supply_chain"] )}</div>
    <div class='fx-label'>🏭 生産・供給網</div>
    <div class='fx-desc'>Production / Supply Chain</div>
  </div>
  <div class='fx-summary-card fx-jpy-strong'>
    <div class='fx-num' style='color:#2ecc71'>{len(cats["regulation"] )}</div>
    <div class='fx-label'>⚖ 政策・規制</div>
    <div class='fx-desc'>Policy / Regulation</div>
  </div>
  <div class='fx-summary-card fx-jpy-weak'>
    <div class='fx-num' style='color:#f39c12'>{len(cats["earnings_market"] )}</div>
    <div class='fx-label'>💹 業績・市場</div>
    <div class='fx-desc'>Earnings / Market</div>
  </div>
...
def render_index(items: list[dict], themes_cfg: dict) -> str:
    ...
    ev_shift = sum(1 for x in items if x.get("fx_impact") == "ev_shift")
    supply_chain   = sum(1 for x in items if x.get("fx_impact") == "supply_chain")
    regulation = sum(1 for x in items if x.get("fx_impact") == "regulation")
    earnings_market   = sum(1 for x in items if x.get("fx_impact") == "earnings_market")
...
<div class='stat-grid'>
  ...
  <div class='stat'><div class='icon'>🟢</div><div class='num' style='color:#4ea1ff'>{ev_shift}</div><div class='label'>EV・電動化</div></div>
  <div class='stat'><div class='icon'>🔴</div><div class='num' style='color:#e74c3c'>{supply_chain}</div><div class='label'>生産・供給網</div></div>
  <div class='stat'><div class='icon'>🔵</div><div class='num' style='color:#2ecc71'>{regulation}</div><div class='label'>政策・規制</div></div>
  <div class='stat'><div class='icon'>🟡</div><div class='num' style='color:#f39c12'>{earnings_market}</div><div class='label'>業績・市場</div></div>
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx' data-fx='ev_shift'>⚡ EV</button>
    <button class='flt-btn flt-fx' data-fx='supply_chain'>🏭 供給網</button>
    <button class='flt-btn flt-fx' data-fx='regulation'>⚖ 規制</button>
    <button class='flt-btn flt-fx' data-fx='earnings_market'>💹 業績・市場</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='ev_shift'>⚡ EV</button>"
        "<button class='flt-btn flt-fx-t' data-fx='supply_chain'>🏭 供給網</button>"
        "<button class='flt-btn flt-fx-t' data-fx='regulation'>⚖ 規制</button>"
        "<button class='flt-btn flt-fx-t' data-fx='earnings_market'>💹 業績・市場</button>"
...
def render_sources(items: list[dict], themes_cfg: dict) -> str:
    ...
    source_cards = [
        ("Response", {"icon": "🇯🇵", "desc": "国内自動車ニュース総合", "group": "jp_media"}),
        ("Car Watch", {"icon": "🇯🇵", "desc": "国内の新車・市場・業界ニュース", "group": "jp_media"}),
        ("Toyota Global Newsroom", {"icon": "🏛", "desc": "トヨタ公式ニュース", "group": "official_oem"}),
        ("Honda Global Newsroom", {"icon": "🏛", "desc": "ホンダ公式ニュース", "group": "official_oem"}),
        ("Nissan Newsroom", {"icon": "🏛", "desc": "日産公式ニュース", "group": "official_oem"}),
        ("Electrek", {"icon": "🌐", "desc": "海外EV・電動化メディア", "group": "ev_media"}),
        ("InsideEVs", {"icon": "🌐", "desc": "海外EV専門メディア", "group": "ev_media"}),
        ("Reuters Automotive", {"icon": "📰", "desc": "海外自動車業界メディア", "group": "industry_media"}),
    ]

    display_sources = []
    for src_name, meta in source_cards:
...
<div class='panel'>
  <div class='filters'>
    ...
    <button class='flt-btn flt-cb-bank' data-bank='jp_media'>🇯🇵 国内メディア</button>
    <button class='flt-btn flt-cb-bank' data-bank='official_oem'>🏛 公式OEM</button>
    <button class='flt-btn flt-cb-bank' data-bank='ev_media'>⚡ 海外EV</button>
    <button class='flt-btn flt-cb-bank' data-bank='industry_media'>📰 海外業界</button>
  </div>
  <div class='filters'>
    <span class='filter-label'>言語:</span>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='ev_shift'>⚡ EV</button>
    <button class='flt-btn flt-fx-cb' data-fx='supply_chain'>🏭 供給網</button>
    <button class='flt-btn flt-fx-cb' data-fx='regulation'>⚖ 規制</button>
    <button class='flt-btn flt-fx-cb' data-fx='earnings_market'>💹 業績・市場</button>
...
(function(){
  ...
  function apply(){
    ...
    document.querySelectorAll('#cbGrid .item-card').forEach(function(card){
      var show=true;
      if(fBank!=='all' && (card.dataset.sourcegroup||'')!==fBank) show=false;
      if(fLang!=='all' && (card.dataset.lang||'')!==fLang) show=false;
      if(fFx!=='all' && (card.dataset.importance||'')!==fFx) show=false;
      if(search && !(card.textContent||'').toLowerCase().includes(search.toLowerCase())) show=false;
      card.style.display=show?'':'none';
  ...
  <div class='two-col'>
    ...
    <div>
      ...
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{fx_counts.get("ev_shift",0)}</div><div style='color:var(--muted);font-size:11px'>⚡ EV</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{fx_counts.get("supply_chain",0)}</div><div style='color:var(--muted);font-size:11px'>🏭 供給網</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{fx_counts.get("regulation",0)}</div><div style='color:var(--muted);font-size:11px'>⚖ 規制</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{fx_counts.get("earnings_market",0)}</div><div style='color:var(--muted);font-size:11px'>💹 業績・市場</div></div>
      </div>
    </div>
  </div>
</div>"""
