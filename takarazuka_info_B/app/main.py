...
DB_FILE = OUTPUT_DIR / 'takarazuka_information_suite.db'
TIMEOUT = 20
...
def is_new(dt: datetime | None) -> bool:
    return bool(dt and (datetime.now(JST).replace(tzinfo=None) - dt) <= timedelta(hours=NEW_BADGE_HOURS))
...
def collect_news_html(session: requests.Session, source: dict) -> list[dict]:
    ...
    for a in soup.find_all('a', href=True):
        ...
        if pub_dt and (datetime.now(JST).replace(tzinfo=None) - pub_dt).days > HISTORY_DAYS:
            continue
...
def collect_all() -> list[dict]:
    ...
    log(f"  🚀 並列取得: {len(SOURCES)} ソース / {max_workers} workers")
    def _fetch_one(source):
        if source['kind'] == 'news_html':
            return source, collect_news_html(session, source)
        elif source['kind'] == 'performance_html':
            return source, collect_performance_html(session, source)
        else:
            return source, []
...
<header><div class='wrap'><div class='top'><div class='title'><h1>{esc(APP_NAME)}</h1><div class='sub'>{esc(SUBTITLE)} / 更新 {updated}</div></div></div><div class='nav'>{nav_html}</div></div></header>
<main class='main'>
<div class='stat-grid'>
