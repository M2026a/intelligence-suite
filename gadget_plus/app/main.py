...
SRC_DESC_JP      = CONFIG.get("source_desc_default_jp", "国内ニュース")
SRC_DESC_GLOBAL  = CONFIG.get("source_desc_default_global", "海外ニュース")
...
DB_FILE              = OUTPUT / CONFIG.get("db_name", "it_plus.db")
USER_AGENT = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 {CONFIG.get('ua_suffix', 'GadgetPlus')}"
TIMEOUT    = 15


def source_desc_from_item_or_name(item_or_name) -> str:
    name = item_or_name if isinstance(item_or_name, str) else item_or_name.get("source", "")
    for src in SOURCES:
        if src.get("name") == name:
            region = str(src.get("region", "")).upper()
            return SRC_DESC_JP if region in ("JP", "JAPAN", "DOMESTIC") else SRC_DESC_GLOBAL
    return SRC_DESC_GLOBAL


def infer_source_icon(name: str, desc: str = "") -> str:
    t = f"{name} {desc}".lower()
    if any(k in t for k in ["nhk", "oricon", "natalie", "eiga", "watch", "itmedia", "日刊", "スポニチ", "デジカメ", "家電"]):
        return "🇯🇵"
    if any(k in t for k in ["espn", "bbc", "sky", "variety", "billboard", "ign", "verge", "engadget", "gizmodo"]):
        return "🌐"
    return "📰"
...
AI_DATA = ["smartphone", "phone", "iphone", "android", "tablet", "ipad", "スマホ", "スマートフォン", "iPhone", "Android", "タブレット", "iPad"]
SECURITY_ALERT = ["pc", "laptop", "monitor", "keyboard", "mouse", "peripheral", "PC", "ノートPC", "モニター", "キーボード", "マウス", "周辺機器"]
CLOUD_PLATFORM = ["camera", "lens", "audio", "earbuds", "headphones", "speaker", "カメラ", "レンズ", "オーディオ", "イヤホン", "ヘッドホン", "スピーカー"]
DEVELOPER_TOOLS = ["smart home", "appliance", "robot vacuum", "air purifier", "home device", "スマート家電", "家電", "ロボット掃除機", "空気清浄機", "ホームデバイス"]
AI_HIGH_VALUE = AI_DATA[:]
AI_LOW_VALUE = []
NON_AI_STRATEGIC = SECURITY_ALERT[:] + CLOUD_PLATFORM[:] + DEVELOPER_TOOLS[:]

def count_keyword_hits(text: str, keywords: list[str]) -> int:
    combined = text_lower(text)
    return sum(1 for kw in keywords if text_lower(kw) in combined)

# 参考度判定用
REFERENCE_WORDS = [
    "official", "documentation", "guide", "research", "study", "release notes", "best practice",
    "whitepaper", "benchmark", "api", "sdk",
    "公式", "ドキュメント", "ガイド", "研究", "調査", "リリースノート", "ベストプラクティス",
    "ホワイトペーパー", "ベンチマーク", "API", "SDK"
]
WARNING_WORDS = [
    "warning", "alert", "risk", "breach", "ransomware", "malware", "exploit", "vulnerability",
    "outage", "incident", "cve",
    "警告", "注意", "危険", "侵害", "ランサムウェア", "マルウェア", "脆弱性",
    "障害", "インシデント", "CVE", "情報漏えい"
]

# スコア加算用イベントキーワード
EVENT_KEYWORDS = {
    3: ["zero-day", "breach", "ransomware", "exploit", "critical vulnerability", "outage", "ゼロデイ", "侵害", "ランサムウェア", "重大脆弱性", "障害"],
    2: ["launch", "release", "general availability", "preview", "funding", "acquisition", "公開", "提供開始", "リリース", "買収", "資金調達"],
    1: ["update", "feature", "benchmark", "integration", "api", "sdk", "アップデート", "新機能", "ベンチマーク", "統合", "API", "SDK"],
}
...
def detect_fx_impact(title: str, summary: str) -> str:
    """重要タグを判定: ai_data / security_alert / cloud_platform / developer_tools / neutral"""
    combined = text_lower(f"{title} {summary}")
    scores = {
        "ai_data": sum(1 for w in AI_DATA if text_lower(w) in combined),
        "security_alert": sum(1 for w in SECURITY_ALERT if text_lower(w) in combined),
        "cloud_platform": sum(1 for w in CLOUD_PLATFORM if text_lower(w) in combined),
        "developer_tools": sum(1 for w in DEVELOPER_TOOLS if text_lower(w) in combined),
    }
...
def infer_theme_from_source(source_name: str) -> list[str]:
    ...
    mapping = []
    for label in list(load_json(SHARED / "themes.json", {}).get("themes", {}).keys()):
        key = text_lower(label.split("・")[0])
        if key and key in src:
            mapping.append(label)
    return mapping if mapping else ["その他"]

def infer_importance_from_source(source_name: str) -> str:
    src = text_lower(source_name)
    if any(k in src for k in ["ai", "machine learning", "llm", "copilot", "openai", "anthropic", "itmedia ai", "zdnet", "techcrunch"]):
        return "ai_data"
    if any(k in src for k in ["security", "bleepingcomputer", "cve", "threat", "vulnerability", "セキュリティ"]):
        return "security_alert"
    if any(k in src for k in ["aws", "azure", "gcp", "cloud", "kubernetes", "infra", "infrastructure"]):
        return "cloud_platform"
    if any(k in src for k in ["infoq", "github", "gitlab", "developer", "devops", "api", "sdk", "programming"]):
    ...
    for points, keywords in EVENT_KEYWORDS.items():
        for kw in keywords:
            if text_lower(kw) in combined:
                event_bonus = max(event_bonus, points)
                break
    score += event_bonus

    ai_value_hits = count_keyword_hits(combined, AI_HIGH_VALUE)
    ai_noise_hits = count_keyword_hits(combined, AI_LOW_VALUE)
    non_ai_hits = count_keyword_hits(combined, NON_AI_STRATEGIC)

    if fx_impact == "ai_data":
        # AIは重要度の見極めを強くする
        if ai_value_hits >= 2:
            score += 2
        elif ai_value_hits == 1:
            score += 1
        if ai_noise_hits >= 2:
            score -= 2
        elif ai_noise_hits == 1:
            score -= 1
        if not item.get("official") and ai_value_hits == 0:
            score -= 1
    else:
        # 非AIの中核ITトピックは厚めに残す
        if non_ai_hits >= 1:
            score += 1
        if fx_impact in {"security_alert", "cloud_platform", "developer_tools"}:
            score += 1

    title = combined
    if ("copilot" in title or "chatgpt" in title or "claude" in title or "gemini" in title):
        if ai_value_hits == 0 and ai_noise_hits >= 1:
            score -= 1
        if any(k in title for k in ["pricing", "license", "governance", "compliance", "security", "導入", "料金", "ライセンス", "ガバナンス"]):
            score += 1
...
def fetch_source(source: dict, themes_cfg: dict) -> tuple[list[dict], bool]:
    ...
    items = []
    for entry in feed.entries:  # 上限なし・日付フィルターで制御
...
.panel{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:16px;margin-bottom:14px;overflow:hidden}
...
.filters{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px;align-items:center;width:100%;max-width:100%;box-sizing:border-box}
.filters > *{min-width:0;max-width:100%;box-sizing:border-box}
.flt-btn{background:var(--panel2);border:1px solid var(--line);color:var(--text);border-radius:10px;padding:7px 13px;font-size:13px;cursor:pointer;transition:all .15s;max-width:100%;box-sizing:border-box}
.flt-btn:hover{border-color:var(--accent)}
.flt-btn.active{background:var(--accent2);border-color:var(--accent);color:#eaf4ff}
.search{background:var(--panel2);border:1px solid var(--line);color:var(--text);border-radius:10px;padding:7px 12px;font-size:13px;min-width:220px;max-width:100%;box-sizing:border-box}
...
.card{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:14px;display:flex;flex-direction:column;gap:10px;transition:border-color .18s,opacity .18s;min-width:0;max-width:100%;box-sizing:border-box}
.card:hover{border-color:var(--accent)}
.card.is-read{opacity:.45}
.card-top{display:flex;justify-content:space-between;align-items:flex-start;gap:8px}
.card-top > *{min-width:0;max-width:100%}
.card-meta{display:flex;gap:6px;flex-wrap:wrap;color:var(--muted);font-size:12px;align-items:center;min-width:0;max-width:100%;overflow-wrap:anywhere;word-break:break-word}
.card-meta span{min-width:0;max-width:100%;overflow-wrap:anywhere;word-break:break-word}
.card-lang-toggle{display:flex;gap:5px;flex-shrink:0;align-items:center;flex-wrap:wrap;min-width:0;max-width:100%}
...
.read-btn{background:var(--panel2);border:1px solid var(--line);color:var(--muted);border-radius:8px;padding:4px 9px;font-size:12px;cursor:pointer;transition:all .15s;max-width:100%}
.read-btn:hover{border-color:#666}
.card h3{margin:0;font-size:15px;line-height:1.5;font-weight:700;overflow-wrap:anywhere;word-break:break-word}
.summary{color:#ced7e6;font-size:13px;line-height:1.6;overflow-wrap:anywhere;word-break:break-word}
.lang-pane.hidden{display:none!important}
.tags{display:flex;flex-wrap:wrap;gap:6px;margin-top:auto;min-width:0;max-width:100%}
.tag{overflow-wrap:anywhere;word-break:break-word;max-width:100%}
.open-btn{max-width:100%;white-space:normal;text-align:center;overflow-wrap:anywhere;word-break:break-word;box-sizing:border-box}
...
@media(max-width:700px){
  ...
  .flt-btn{white-space:normal;overflow-wrap:anywhere;word-break:break-word}
  #sourceFilterRow .flt-source{white-space:normal;overflow-wrap:anywhere;word-break:break-word}
  .card-top{flex-direction:column}
  .card-lang-toggle{width:100%}
}
"""
...
<div class='wrap'>
  <div class='top'>
    <div class='title'>
      <h1>{APP_ICON} {html.escape(APP_NAME)} <span style='font-weight:400;color:var(--muted);font-size:16px'>｜ {html.escape(CONFIG.get("subtitle", ""))}</span></h1>
...
IMPORTANCE_MAP = {
    "ai_data": (f"{FX_ICONS.get('ai_data','🏷️')} {FX_LABELS.get('ai_data','カテゴリ1')}", "tag-usd-strong"),
    "security_alert": (f"{FX_ICONS.get('security_alert','🏷️')} {FX_LABELS.get('security_alert','カテゴリ2')}", "tag-usd-weak"),
    "cloud_platform": (f"{FX_ICONS.get('cloud_platform','🏷️')} {FX_LABELS.get('cloud_platform','カテゴリ3')}", "tag-jpy-strong"),
    "developer_tools": (f"{FX_ICONS.get('developer_tools','🏷️')} {FX_LABELS.get('developer_tools','カテゴリ4')}", "tag-jpy-weak"),
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    cats = {
        "ai_data": [x for x in items if x.get("fx_impact") == "ai_data"],
        "security_alert": [x for x in items if x.get("fx_impact") == "security_alert"],
        "cloud_platform": [x for x in items if x.get("fx_impact") == "cloud_platform"],
        "developer_tools": [x for x in items if x.get("fx_impact") == "developer_tools"],
    }
    neutral_count = sum(1 for x in items if x.get("fx_impact") == "neutral")

    tab_cfg = [
        ("ai_data", "📱 スマホ・タブレット", "#7c4dff", "#1a1233"),
        ("security_alert", "💻 PC・周辺機器", "#ff5c5c", "#2f1111"),
        ("cloud_platform", "📷 カメラ・オーディオ", "#00a8ff", "#0d1e35"),
        ("developer_tools", "🏠 スマート家電", "#20bf6b", "#0d2e1a"),
    ]
...
.fx-usd-strong{background:#1a1233;border-color:#7c4dff}
.fx-usd-weak{background:#2f1111;border-color:#ff5c5c}
.fx-jpy-strong{background:#0d1e35;border-color:#00a8ff}
.fx-jpy-weak{background:#0d2e1a;border-color:#20bf6b}
...
<div class='fx-summary-grid'>
  <div class='fx-summary-card fx-usd-strong'>
    <div class='fx-num' style='color:#7c4dff'>{len(cats["ai_data"])}</div>
    <div class='fx-label'>📱 スマホ・タブレット</div>
    <div class='fx-desc'>Phones / Tablets</div>
  </div>
  <div class='fx-summary-card fx-usd-weak'>
    <div class='fx-num' style='color:#ff5c5c'>{len(cats["security_alert"])}</div>
    <div class='fx-label'>💻 PC・周辺機器</div>
    <div class='fx-desc'>PC / Peripherals</div>
  </div>
  <div class='fx-summary-card fx-jpy-strong'>
    <div class='fx-num' style='color:#00a8ff'>{len(cats["cloud_platform"])}</div>
    <div class='fx-label'>📷 カメラ・オーディオ</div>
    <div class='fx-desc'>Camera / Audio</div>
  </div>
  <div class='fx-summary-card fx-jpy-weak'>
    <div class='fx-num' style='color:#20bf6b'>{len(cats["developer_tools"])}</div>
    <div class='fx-label'>🏠 スマート家電</div>
    <div class='fx-desc'>Smart Appliances</div>
  </div>
...
function switchFxTab(key, btn){{
  ...
  var panel = document.getElementById('fx-' + key);
...
def render_index(items: list[dict], themes_cfg: dict) -> str:
    ...
    ai_data = sum(1 for x in items if x.get("fx_impact") == "ai_data")
    security_alert = sum(1 for x in items if x.get("fx_impact") == "security_alert")
    cloud_platform = sum(1 for x in items if x.get("fx_impact") == "cloud_platform")
    developer_tools = sum(1 for x in items if x.get("fx_impact") == "developer_tools")
...
<div class='stat-grid'>
  ...
  <div class='stat'><div class='icon'>🆕</div><div class='num' style='color:#20bf6b'>{new_count}</div><div class='label'>直近{NEW_HOURS}h 新着</div></div>
  <div class='stat'><div class='icon'>🤖</div><div class='num' style='color:#7c4dff'>{ai_data}</div><div class='label'>{FX_LABELS.get('ai_data','カテゴリ1')}</div></div>
  <div class='stat'><div class='icon'>🔐</div><div class='num' style='color:#ff5c5c'>{security_alert}</div><div class='label'>{FX_LABELS.get('security_alert','カテゴリ2')}</div></div>
  <div class='stat'><div class='icon'>☁️</div><div class='num' style='color:#00a8ff'>{cloud_platform}</div><div class='label'>{FX_LABELS.get('cloud_platform','カテゴリ3')}</div></div>
  <div class='stat'><div class='icon'>🛠️</div><div class='num' style='color:#20bf6b'>{developer_tools}</div><div class='label'>{FX_LABELS.get('developer_tools','カテゴリ4')}</div></div>
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx' data-fx='ai_data'>📱 スマホ・タブレット</button>
    <button class='flt-btn flt-fx' data-fx='security_alert'>💻 PC・周辺機器</button>
    <button class='flt-btn flt-fx' data-fx='cloud_platform'>📷 カメラ・オーディオ</button>
    <button class='flt-btn flt-fx' data-fx='developer_tools'>🏠 スマート家電</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='ai_data'>📱 スマホ・タブレット</button>"
        "<button class='flt-btn flt-fx-t' data-fx='security_alert'>💻 PC・周辺機器</button>"
        "<button class='flt-btn flt-fx-t' data-fx='cloud_platform'>📷 カメラ・オーディオ</button>"
        "<button class='flt-btn flt-fx-t' data-fx='developer_tools'>🏠 スマート家電</button>"
...
def render_sources(items: list[dict], themes_cfg: dict) -> str:
    ...
    for src in SOURCES:
        name = src.get("name", "")
        region = str(src.get("region", "")).upper()
        desc = SRC_DESC_JP if region in ("JP", "JAPAN", "DOMESTIC") else SRC_DESC_GLOBAL
        source_meta[name] = {"icon": infer_source_icon(name, desc), "desc": desc}

    seen_names = {x.get("source", "") for x in items if x.get("source")}
    for src_name in sorted(seen_names):
        if src_name not in source_meta:
            desc = source_desc_from_item_or_name(src_name)
            source_meta[src_name] = {"icon": infer_source_icon(src_name, desc), "desc": desc}
...
<div class='cb-card'>
  ...
  <div class='cb-recent'>最新: {html.escape(latest)}</div>
    ...
    bank_buttons = "".join(
        f"<button class='flt-btn flt-cb-bank' data-bank='{html.escape(src)}'>{meta['icon']} {html.escape(src)}</button>"
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='ai_data'>📱 スマホ・タブレット</button>
    <button class='flt-btn flt-fx-cb' data-fx='security_alert'>💻 PC・周辺機器</button>
    <button class='flt-btn flt-fx-cb' data-fx='cloud_platform'>📷 カメラ・オーディオ</button>
    <button class='flt-btn flt-fx-cb' data-fx='developer_tools'>🏠 スマート家電</button>
...
(function(){{
  ...
  document.querySelectorAll('.flt-cb-bank').forEach(function(btn){{btn.addEventListener('click',function(){{document.querySelectorAll('.flt-cb-bank').forEach(function(b){{b.classList.remove('active');}});btn.classList.add('active');fBank=btn.dataset.bank||'all';apply();}});}});
  document.querySelectorAll('.flt-lang-cb').forEach(function(btn){{btn.addEventListener('click',function(){{document.querySelectorAll('.flt-lang-cb').forEach(function(b){{b.classList.remove('active');}});btn.classList.add('active');fLang=btn.dataset.lang||'all';apply();}});}});
  document.querySelectorAll('.flt-fx-cb').forEach(function(btn){{btn.addEventListener('click',function(){{document.querySelectorAll('.flt-fx-cb').forEach(function(b){{b.classList.remove('active');}});btn.classList.add('active');fFx=btn.dataset.fx||'all';apply();}});}});
  document.querySelectorAll('.flt-view-cb').forEach(function(btn){{btn.addEventListener('click',function(){{document.querySelectorAll('.flt-view-cb').forEach(function(b){{b.classList.remove('active');}});btn.classList.add('active');fView=btn.dataset.view||'cards';apply();}});}});
...
<div class='main'>
  <div class='panel' style='margin-bottom:16px'>
    <h2 style='margin:0 0 14px;font-size:16px'>🖥️ 注目ソースモニター</h2>
  ...
  <div class='two-col'>
    ...
    <div>
      ...
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{fx_counts.get("ai_data",0)}</div><div style='color:var(--muted);font-size:11px'>📱 スマホ・タブレット</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{fx_counts.get("security_alert",0)}</div><div style='color:var(--muted);font-size:11px'>💻 PC・周辺機器</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{fx_counts.get("cloud_platform",0)}</div><div style='color:var(--muted);font-size:11px'>📷 カメラ・オーディオ</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{fx_counts.get("developer_tools",0)}</div><div style='color:var(--muted);font-size:11px'>🏠 スマート家電</div></div>
      </div>
    </div>
  </div>
</div>"""
