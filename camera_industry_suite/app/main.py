...
# ─── 旧取得ロジックローダー ─────────────────────────────────────
def load_legacy_fetch_module():
    import importlib.util
    legacy_path = Path(__file__).with_name("legacy_fetch.py")
...
DB_FILE              = OUTPUT / "camera_industry_suite.db"
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 CameraIndustrySuite"
TIMEOUT    = 15

# ─── 重要タグキーワード ───────────────────────────────────────

NEW_PRODUCT = [
    "new", "launch", "announce", "announced", "release", "released", "coming", "debut",
    "camera", "lens", "body", "mirrorless", "dslr", "compact", "firmware", "sensor", "autofocus",
    "新製品", "新型", "発表", "発売", "登場", "正式発表", "開発発表", "予約開始", "供給", "出荷",
    "カメラ", "レンズ", "ボディ", "ミラーレス", "一眼レフ", "コンパクト", "ファームウェア", "センサー", "AF"
]
RUMOR_LEAK = [
    "rumor", "rumors", "leak", "leaked", "specs", "specification", "expected", "coming soon", "first images",
    "噂", "うわさ", "リーク", "流出", "予想", "予測", "スペック", "仕様", "登場か", "近日", "画像流出"
]
REVIEW_SAMPLE = [
    "review", "hands-on", "preview", "first look", "sample gallery", "sample images", "field test", "comparison", "image quality", "test",
    "レビュー", "実写", "作例", "ハンズオン", "比較", "画質", "使用感", "テスト", "検証", "サンプル"
]
COMPANY_MARKET = [
    "company", "business", "earnings", "financial", "factory", "production", "shipment", "supply chain", "market", "industry", "creator", "photography business",
    "企業", "決算", "業績", "工場", "生産", "出荷", "供給網", "市場", "業界", "写真文化", "クリエイター", "事業"
]

# 参考度判定用
REFERENCE_WORDS = [
    "official", "review", "hands-on", "sample", "comparison", "tested", "analysis", "interview", "preview",
    "公式", "レビュー", "実写", "作例", "比較", "検証", "分析", "インタビュー", "先行"
]
WARNING_WORDS = [
    "delay", "shortage", "recall", "issue", "problem", "discontinued", "price increase", "lawsuit", "supply constraint",
    "遅延", "供給不足", "不具合", "問題", "回収", "生産終了", "値上げ", "訴訟", "供給制約"
]

# スコア加算用イベントキーワード
EVENT_KEYWORDS = {
    3: ["announced", "launch", "new product", "rumor", "leak", "recall", "shortage", "新製品", "発表", "発売", "リーク", "噂", "不具合", "供給不足"],
    2: ["review", "hands-on", "sample gallery", "earnings", "factory", "production", "レビュー", "実写", "作例", "決算", "工場", "生産"],
    1: ["firmware", "autofocus", "sensor", "comparison", "creator", "ファームウェア", "AF", "センサー", "比較", "写真文化"],
}
...
def fetch_feed_content(src: dict) -> tuple[str, bytes]:
    headers = {
    ...
    for candidate in list(dict.fromkeys(candidates)):
        try:
            referer = "https://videocardz.com/" if "videocardz.com" in candidate else "https://www.google.com/"
            resp = requests.get(candidate, headers={**headers, "Referer": referer}, timeout=20)
    ...
    if last_err is not None:
        raise last_err
    raise RuntimeError(f"feed fetch failed: {src.get('name', '')}")
...
def detect_fx_impact(title: str, summary: str) -> str:
    """重要タグを判定: new_product / rumor_leak / review_sample / company_market / neutral"""
    combined = text_lower(f"{title} {summary}")
    scores = {
        "health_alert": sum(1 for w in NEW_PRODUCT if text_lower(w) in combined),
        "food_recall": sum(1 for w in RUMOR_LEAK if text_lower(w) in combined),
        "training_behavior": sum(1 for w in REVIEW_SAMPLE if text_lower(w) in combined),
        "breed_adoption": sum(1 for w in COMPANY_MARKET if text_lower(w) in combined),
    }
...
def infer_theme_from_source(source_name: str) -> list[str]:
    ...
    mapping = [
        (("nikon", "canon", "sony", "fujifilm", "panasonic", "om system", "ricoh", "sigma", "tamron", "viltrox", "firmware", "announce", "release", "new"), "新製品"),
        (("rumor", "leak", "rumors", "リーク", "噂"), "噂・リーク"),
        (("review", "hands-on", "sample", "gallery", "preview", "レビュー", "作例", "実写"), "レビュー・作例"),
        (("company", "financial", "earnings", "factory", "production", "market", "industry", "企業", "決算", "市場", "生産"), "企業動向"),
        (("photo", "photography", "creator", "copyright", "gallery", "写真", "写真文化", "著作権", "クリエイター"), "市場・写真文化"),
...
def infer_importance_from_source(source_name: str) -> str:
    src = text_lower(source_name)
    if any(k in src for k in ["camera", "lens", "nikon", "canon", "sony", "fujifilm", "panasonic", "om system", "firmware", "new", "announce", "release"]):
        return "health_alert"
    if any(k in src for k in ["rumor", "rumors", "leak", "リーク", "噂"]):
        return "food_recall"
    if any(k in src for k in ["review", "hands-on", "preview", "sample", "gallery", "レビュー", "作例", "実写"]):
        return "training_behavior"
    if any(k in src for k in ["company", "financial", "earnings", "factory", "production", "market", "industry", "企業", "決算", "市場", "生産"]):
...
def fetch_source(source: dict, themes_cfg: dict) -> tuple[list[dict], bool]:
    ...
    log(f"  取得中: {name}")
    try:
        fetched_url, content = fetch_feed_content(source)
...
def read_js() -> str:
    return """
<script>
var READ_KEY = 'camera_industry_suite_read';
function getReadSet() {
...
IMPORTANCE_MAP = {
    "health_alert": ("📷 新製品・発表", "tag-usd-strong"),
    "food_recall": ("👀 噂・リーク", "tag-usd-weak"),
    "training_behavior": ("🖼 レビュー・作例", "tag-jpy-strong"),
    "breed_adoption": ("🏢 企業・市場", "tag-jpy-weak"),
...
def render_focus(items: list[dict], themes_cfg: dict) -> str:
    ...
    tab_cfg = [
        ("health_alert", "📷 新製品・発表", "#4ea1ff", "#0d1e35"),
        ("food_recall",   "👀 噂・リーク", "#e74c3c", "#2e0d0d"),
        ("training_behavior", "🖼 レビュー・作例",   "#2ecc71", "#0d2e1a"),
        ("breed_adoption",   "🏢 企業・市場",   "#f39c12", "#2e2400"),
    ]
...
def render_index(items: list[dict], themes_cfg: dict) -> str:
    ...
    new_products_count = theme_counts.get("新製品", 0)
    rumors_count = theme_counts.get("噂・リーク", 0)
    reviews_count = theme_counts.get("レビュー・作例", 0)
    company_count = theme_counts.get("企業動向", 0)
    market_count = theme_counts.get("市場・写真文化", 0)
    other_count = theme_counts.get("その他", 0)
...
<div class='panel'>
  ...
  <div class='filters'>
    ...
    <span class='filter-label'>注目分類:</span>
    <button class='flt-btn flt-fx active' data-fx='all'>全部</button>
    <button class='flt-btn flt-fx' data-fx='health_alert'>📷 新製品</button>
    <button class='flt-btn flt-fx' data-fx='food_recall'>👀 噂</button>
    <button class='flt-btn flt-fx' data-fx='training_behavior'>🖼 レビュー</button>
    <button class='flt-btn flt-fx' data-fx='breed_adoption'>🏢 企業・市場</button>
  </div>
  <div class='filters'>
...
<div class='theme-section' data-section='{html.escape(tk)}'>
  ...
  <div class='grid theme-grid'>{cards}</div>
    ...
    filter_bar = (
        ...
        "<button class='flt-btn flt-fx-t' data-fx='health_alert'>📷 新製品</button>"
        "<button class='flt-btn flt-fx-t' data-fx='food_recall'>👀 噂</button>"
        "<button class='flt-btn flt-fx-t' data-fx='training_behavior'>🖼 レビュー</button>"
        "<button class='flt-btn flt-fx-t' data-fx='breed_adoption'>🏢 企業・市場</button>"
...
def render_sources(items: list[dict], themes_cfg: dict) -> str:
    stamp = datetime.now(JST).strftime("%Y-%m-%d %H:%M")
    source_meta = {
        "デジカメ Watch": {"icon": "🇯🇵", "desc": "国内カメラ系メディア"},
        "ニコンイメージング": {"icon": "🏛", "desc": "ニコン公式"},
        "ソニー α 国内": {"icon": "🏛", "desc": "ソニー公式"},
        "Panasonic Newsroom Japan": {"icon": "🏛", "desc": "パナソニック公式"},
        "DPReview": {"icon": "🌐", "desc": "海外レビュー系メディア"},
        "PetaPixel": {"icon": "🌐", "desc": "海外カメラ系メディア"},
        "Photo Rumors": {"icon": "🌐", "desc": "海外カメラ系メディア"},
        "Nikon Rumors": {"icon": "👀", "desc": "Nikon系Rumor"},
        "Canon Rumors": {"icon": "👀", "desc": "Canon系Rumor"},
        "Sony Alpha Rumors": {"icon": "👀", "desc": "Sony系Rumor"},
        "Fuji Rumors": {"icon": "👀", "desc": "Fuji系Rumor"},
        "43 Rumors": {"icon": "👀", "desc": "MFT系Rumor"},
        "Leica Rumors": {"icon": "👀", "desc": "Leica系Rumor"},
    }

    def _safe_source_latest(src_name: str, src_items: list[dict]) -> str:
        if not src_items:
            return "記事なし"

        def _valid_year(s: str) -> bool:
            try:
                return int((s or "")[:4]) >= 2020
            except Exception:
                return False

        valid_items = [x for x in src_items if _valid_year(x.get("published", ""))]
        if valid_items:
            valid_items.sort(key=lambda x: x.get("pub_dt", x.get("published", "")), reverse=True)
            return valid_items[0].get("published", "記事なし") or "記事なし"

        if src_name == "ソニー α 国内":
            return "日付要確認"

        fallback = sorted(src_items, key=lambda x: x.get("pub_dt", x.get("published", "")), reverse=True)
        return fallback[0].get("published", "記事なし") or "記事なし"

    display_sources = []
    for src_name, meta in source_meta.items():
        src_items = [x for x in items if x.get("source") == src_name]
        latest = _safe_source_latest(src_name, src_items)
...
<div class='panel'>
  <div class='filters'>
    ...
    <button class='flt-btn flt-cb-bank' data-bank='デジカメ Watch'>🇯🇵 デジカメ Watch</button>
    <button class='flt-btn flt-cb-bank' data-bank='ニコンイメージング'>🏛 Nikon</button>
    <button class='flt-btn flt-cb-bank' data-bank='ソニー α 国内'>🏛 Sony</button>
    <button class='flt-btn flt-cb-bank' data-bank='Panasonic Newsroom Japan'>🏛 Panasonic</button>
    <button class='flt-btn flt-cb-bank' data-bank='DPReview'>🌐 DPReview</button>
    <button class='flt-btn flt-cb-bank' data-bank='PetaPixel'>🌐 PetaPixel</button>
    <button class='flt-btn flt-cb-bank' data-bank='Canon Rumors'>👀 Canon Rumors</button>
    <button class='flt-btn flt-cb-bank' data-bank='Sony Alpha Rumors'>👀 Sony Rumors</button>
  </div>
  <div class='filters'>
    <span class='filter-label'>言語:</span>
    ...
    <button class='flt-btn flt-fx-cb' data-fx='health_alert'>📷 新製品</button>
    <button class='flt-btn flt-fx-cb' data-fx='food_recall'>👀 噂</button>
    <button class='flt-btn flt-fx-cb' data-fx='training_behavior'>🖼 レビュー</button>
    <button class='flt-btn flt-fx-cb' data-fx='breed_adoption'>🏢 企業・市場</button>
...
<div class='main'>
  <div class='panel' style='margin-bottom:16px'>
    <h2 style='margin:0 0 14px;font-size:16px'>📡 ソース別モニター</h2>
...
def render_analysis(items: list[dict], themes_cfg: dict) -> str:
    ...
    sent_html = f"""
<div class='panel' style='margin-bottom:16px'>
  <h2 style='margin:0 0 14px;font-size:16px'>📊 参考度・テーマ分類サマリー</h2>
  <div class='two-col'>
    <div>
    ...
    <div>
      <div style='font-size:13px;color:var(--muted);margin-bottom:10px'>テーマ件数</div>
      <div style='display:flex;gap:14px;flex-wrap:wrap'>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#4ea1ff'>{theme_counts.get("新製品",0)}</div><div style='color:var(--muted);font-size:11px'>📷 新製品</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#e74c3c'>{theme_counts.get("噂・リーク",0)}</div><div style='color:var(--muted);font-size:11px'>👀 噂・リーク</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#2ecc71'>{theme_counts.get("レビュー・作例",0)}</div><div style='color:var(--muted);font-size:11px'>🖼 レビュー・作例</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#f39c12'>{theme_counts.get("企業動向",0)}</div><div style='color:var(--muted);font-size:11px'>🏢 企業動向</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#a29bfe'>{theme_counts.get("市場・写真文化",0)}</div><div style='color:var(--muted);font-size:11px'>🌍 市場・写真文化</div></div>
        <div style='text-align:center'><div style='font-size:24px;font-weight:800;color:#9ba9bc'>{theme_counts.get("その他",0)}</div><div style='color:var(--muted);font-size:11px'>🏷 その他</div></div>
      </div>
    </div>
  </div>
</div>"""
...
def main() -> None:
    ...
    legacy_fetch = load_legacy_fetch_module()
    legacy_items, _legacy_themes, _legacy_brands, legacy_stats = legacy_fetch.fetch_all()
    ...
    for raw in legacy_items:
        ...
        themes = classify_themes(title, summary, lang, themes_cfg)
        if themes == ["その他"]:
            themes = infer_theme_from_source(item.get("source", ""))
        item["themes"] = themes
        item["sentiment"] = detect_sentiment(title, summary)
        fx_impact = detect_fx_impact(title, summary)
        if fx_impact == "neutral":
            fx_impact = infer_importance_from_source(item.get("source", ""))
        item["fx_impact"] = fx_impact
        item["is_new"] = is_new(item.get("pub_dt", ""))
        item["official"] = bool(item.get("official", False))
        item["region"] = item.get("region", "")
        item["score"] = item.get("score", calc_score(item))
        items.append(item)

    ok = sum(1 for row in legacy_stats.get("source_stats", []) if row.get("status") == "ok")
    err = sum(1 for row in legacy_stats.get("source_stats", []) if row.get("status") != "ok")
