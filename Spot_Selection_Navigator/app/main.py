...
def page_index() -> str:
    ...
    css = (
        ...
        ".topbar{background:#111827;border-bottom:1px solid var(--line);padding:10px 20px;position:sticky;top:0;z-index:50;width:1060px;margin:0 auto}"
        ...
        ".tab-wrap{background:#0e1628;border-bottom:1px solid var(--line);padding:8px 16px;overflow-x:visible;white-space:normal;position:sticky;top:49px;z-index:40;width:1060px;margin:0 auto}"
        ...
        ".wrap{width:1100px;margin:0 auto;padding:18px 20px;overflow-x:hidden}"
        ...
        ".footer{margin:24px auto 0;padding:14px 20px;color:var(--muted);font-size:12px;width:1060px}"
    ...
    return (
        ...
        "\n"
        "<div class=\"topbar\">\n"
        "  <div class=\"topbar-title\">" + e(APP_ICON) + " " + e(APP_NAME) + "<span>| " + e(APP_SUB) + "</span></div>\n"
        "  <div class=\"topbar-meta\">更新日時：" + e(UPDATED_AT) + "</div>\n"
        "</div>\n"
        "\n"
        ...
        "<input id=\"area\" list=\"area-list\" placeholder=\"例: 栄 / 名古屋駅\">"
        "<datalist id=\"area-list\"></datalist></div>\n"
        "      <div><label for=\"subcat\">種類</label>"
        "<select id=\"subcat\" onchange=\"applyLinks()\">" + subcat_opts + "</select></div>\n"
        "      <div><label for=\"scene\">シーン</label>"
        ...
        "      <div class=\"query-display\">現在の検索語：<strong id=\"queryPreview\">未入力</strong></div>\n"
        "    </div>\n"
        "  </div>\n"
        "\n"
        "  <div class=\"card\">\n"
        ...
        "    <div>営業情報は外部サイトで確認を</div>\n"
        "    <div>各トップページ：<span id=\"topLinks\"></span></div>\n"
        "  </div>\n"
        "\n"
        "</div>\n"
        "\n"
        "<div class=\"footer\">" + e(APP_NAME) + " — Generated at " + e(UPDATED_AT) + "</div>\n"
        "\n"
        "<script>\n" + js + "</script>\n"
        "</body>\n"
        "</html>"
...
def main() -> None:
    start = time.time() 
    print("==============================================")
    print("  Spot_Selection_Navigator")
    print("==============================================")
